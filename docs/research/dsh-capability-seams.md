...
| **MCP integration** | Ship-of-theseus point: each MCP server = one `@deepseek-ai/dsh-mcp-client` plugin instance in `cordis.yml` (stdio or streamable-http), registering tools as `mcp__<server>__<rawName>` on `ctx.tools`. Config-only today (no runtime add/remove API) — an opportunity, not a limitation, for `/mcp`. | `packages/mcp/mcp-client/README.md` (usage YAML + naming rules); CLI note "no MCP server is enabled by default because each server command is trusted executable code outside the agent sandbox" in `apps/cli/reference/README.md` |
...
**Git-install catch (critical for our installer UX):** a git install fetches *sources*; pnpm ≥10 blocks the `prepare` build script until the user allowlists it in the profile's `pnpm-workspace.yaml` (`allowBuilds: { <pkg>: true }`) and re-runs. The docs themselves call this allowance "permission to execute the package's code on your machine at install time, outside any sandbox" and recommend pinning `github:owner/repo#<sha>` (`basic/publish.md` §Installing from GitHub). Our installer should surface exactly this warning + pinning.
...
Trust-scan inputs we can harvest without any upstream change: full source tree of each installed package (it's on disk), manifest + patch layers, `prepare`/build scripts (install-time execution risk — the docs' own words about `allowBuilds`), dynamic-eval signals (`new Function`, `eval`, `child_process` — charter's checklist), declared `Config` schemas, and lifecycle hook listeners (event names appear in source). This feeds the adversarial review + report card pipeline in `docs/audits/`.
...
3. **MCP runtime management** — adding/removing an MCP server means editing patch rows + reload; is there a supported hot-add path via `dynamicCordisRunner`, or must `/bridge:mcp` rewrite the profile patch and prompt a reload? *(verify)*
