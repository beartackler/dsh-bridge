...
**Lifecycle guarantees that matter to us:** every registration through `ctx` is undone on unload (`docs/user/develop/framework/index.md` — fiber state machine PENDING→LOADING→ACTIVE→UNLOADING→DISPOSED/FAILED); dependency-driven loading waits for `inject`ed services and auto-disposes dependents when a service disappears; HMR hot-replaces plugins on config edit (`docs/user/develop/basic/config.md` §Work with HMR). The security consequence of all three is load-bearing enough to get its own section below.

### 1.1 Why the trust layer exists: plugins execute inside the host process

Every plugin's registrations and effects run **inside the host process itself**: there is no per-plugin OS sandbox, so sandboxing is per-tool-call, not per-plugin (lifecycle mechanics cited above). A malicious plugin therefore reaches what the host reaches — filesystem, network, credential resolution — bounded only by whatever tool-call policies the composition happens to install. This is the technical foundation of dsh-bridge's killer feature, not a footnote: the install-time allowance that puts plugin code on the machine (`allowBuilds`, §2) and the in-process execution that follows loading are two halves of one trust boundary, argued in full in `dsh-native-inventory.md` §12.4.
...
- Revision 1b (2026-08-26): the in-process-execution security consequence that trailed the §1 lifecycle paragraph is promoted to its own subsection (§1.1), cross-referencing the install-time argument in `dsh-native-inventory.md` §12.4 (reviewer finding 2).
