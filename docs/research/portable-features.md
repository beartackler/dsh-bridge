...
| 2 | `/model` — switch model mid-session, show current | CC, Codex (`/model`, `--model`), OpenCode, Jcode | **S** | **3** | Native `/model` exists (`ui-model-selection`, client-side popupSelect with a two-level Model/Effort menu) and the inventory doc verdicts it **SKIP** (alias only). Bridge value is familiar-name aliasing (`/model sonnet` → DSH route), which is post-MVP polish, not port work. Value lowered 5 → 3 in Revision 1 to match the SKIP verdict; revisit only with evidence the native picker blocks migration. |
...
| 7 | Plan mode / read-only mode toggle | CC (Shift+Tab plan mode), Codex (`--sandbox read-only`), Jcode | **S** | **5** | DSH ships `dsh-plan-mode` (`/plan [message]`). Permission presets are exactly `workspace-write` and `danger-full-access`; `read-only` is a sandbox mode (the fail-safe default), `ask` an approval policy — neither is a preset name. Mostly an *alias + keybinding + status indicator* job, not new machinery. |
...
| 12 | Custom slash commands from markdown files | CC (`.claude/commands/*.md`, `$ARGUMENTS`), OpenCode, Jcode skills | **L** | **4** | DSH `ctx.skills` + `skill-filesystem` scans `.dsh/skills` and `.agents/skills`, not `.claude/commands`; making existing trees work needs custom roots or copy/symlink plus a frontmatter translator (`description`, `allowed-tools`, `argument-hint`, `model`; `$ARGUMENTS`/`$1` substitution). Scope as a converter/importer command. Difficulty raised M → L in Revision 1: more than "state, file I/O, or a UI slot". |
...
| 16 | Session cost / token usage display | CC (`/cost`, `/status`), Codex (`/status`), OpenCode | **S** | **4** | DSH has `token-meter` (`packages/llm/token-meter`) and `session-stats`; no `/status` command registration exists anywhere in the checkout *(verify)*. Port `/cost` as an alias with per-model pricing table in config. |
...
| 22 | `/theme` — light/dark/custom | CC, Codex, OpenCode, Jcode | **S** | **3** | Theme system is native (`packages/client/ui-theme`); no `/theme` command registration was found — switching happens on the settings surface, so an alias would be client-side work *(verify)*. Skip; only ensure BRAND_GUIDELINES-compliant bridge UI respects it. |
| 23 | `/status` / `/doctor` — env & config diagnostics | CC (`/status`, `/doctor`), Codex (`/status`) | **M** | **4** | DSH has `runtime-diagnostics/invariants` but no `/status` command (see row 16), so `/doctor` is net-new. A *great* first-run trust artifact and pairs with the connectors flow. |
| 24 | Bash passthrough (`!cmd`) and output capture | CC (`!` prefix), OpenCode | **S** | **5** | Composer `leadingInput` command kind exists in `ui-commands` — a natural fit *(verify `!` can be registered; the parser requires a leading `/`)*. If `!` is unreachable, offer `/sh`. Value raised 3 → 5 in Revision 1: among the highest-frequency CC keystrokes, scored below `/export` before without justification. |
...
| 29 | TODO/task list rendering | CC (TodoWrite), Jcode (`todo`) | **S** | **2** | **Already native**: `tool-todo` owns state; the checklist renderer lives in `ui-conversation` (`TodoPanel.tsx`), while `ui-plan` provides the plan-mode control (corrected in Revision 1). |
...
4. **`/compact`** — already native; verify `<focus-instructions>` arg-passthrough parity and stop there. (Replaced `/model`, demoted to post-MVP aliasing per Revision 1: the inventory doc verdicts native `/model` SKIP.)
5. **Permission presets with familiar flag names** — `--dangerously-skip-permissions`, `--full-auto`, `--yolo`, `--ask-for-approval` mapped onto DSH presets, with plan mode reachable by Shift+Tab. (Post-MVP follow-up: `/model` familiar-name aliases.)
...
- ~~Namespacing: `/bridge:install` implies colon-namespaced commands; confirm whether `:` is legal in a command name.~~ **Resolved (Revision 1): no.** `parseCommand()` regexes (`packages/interaction/commands/src/index.ts:28,117`) accept `[a-z][a-z0-9_-]*` only, so `/bridge:install` is unparseable; use `/bridge-install` or a single `/bridge` with subcommand grammar in `rawInput`. Matches `dsh-native-inventory.md` §1.1/§14. The command-naming decision is unblocked.
...
- Revision 1b (2026-08-26, second review pass):
  - `/model`: value lowered 5 → 3 and removed from the MVP cut (row 2, MVP item 4). The inventory doc verdicts native `/model` **SKIP**; the contradiction is resolved in favor of the inventory. Familiar-name aliasing moves to post-MVP.
  - Row 7: invented preset names corrected against `packages/interaction/permission-presets/src/index.ts:184-190`. Shipped presets are `workspace-write` and `danger-full-access`; `read-only` is a sandbox mode (`sandbox-policy/src/index.ts`, fail-safe default) and `ask` an approval policy (`user-approval/src/index.ts:97`), not preset names.
  - Rows 16/22/23: unverified command names annotated or dropped. No `/status` or `/theme` command registration exists anywhere in the checkout (re-enumerated); package paths kept.
  - Row 24 (bash passthrough): value raised 3 → 5 per the reviewer's re-scoring note.
  - Row 29: renderer citation corrected; the todo checklist renders via `ui-conversation`'s `TodoPanel.tsx`; `ui-plan` is the plan-mode control.
  - Open question on colon namespacing marked resolved with citations (`commands/src/index.ts:28,117`).
