...
const VALUE_RANK = { high: 3, medium: 2, low: 1 };
/**
 ...
 */
export function classifyLines(content) {
...
// Detectors
// ---------------------------------------------------------------------------
const FUNCTION_START = /(^|\s)(function\s+[A-Za-z0-9_$]*|[A-Za-z0-9_$]+\s*\([^)]*\)\s*\{|=>\s*\{|=\s*async\s*\()/;
 ...
 */
const BLOCK_OPENER = /(^|[^A-Za-z0-9_$])(if|else|for|while|switch|case|try|catch|finally|do|function|class|=>)\b|\)\s*\{\s*$/;
/** `name(` capture for the message; falls back to an anonymous label. */
function functionLabel(code) {
...
function scanStructure(path, lines) {
    ...
    for (let index = 0; index < lines.length; index += 1) {
        ...
        for (let brace = 0; brace < opens; brace += 1)
            braces.push(isBlock);
        ...
        for (let brace = 0; brace < closes; brace += 1)
            braces.pop();
        while (functionStack.length > 0 &&
            blockDepth() <= functionStack[functionStack.length - 1].depth) {
...
export function resolveTargets(deps, target) {
    const stat = deps.statPath(target);
    if (stat === null)
        return { files: [], skipped: [{ path: target, reason: "not found" }] };
    if (stat.isFile) {
    ...
    for (const name of [...deps.readDir(target)].sort()) {
        ...
        if (childStat === null || !childStat.isFile)
            continue;
        if (isSupported(child))
            files.push(child);
        else
            skipped.push({ path: child, reason: "unsupported extension" });
    }
...
export function auditTargets(deps, options, cwd) {
    const files = [];
    const skipped = [];
    if (options.diff) {
        ...
        const filtered = typeof target === "string"
    ...
    const kept = rankFindings(raw).filter((finding) => VALUE_RANK[finding.value] >= VALUE_RANK[options.minValue]);
    return {
...
export function renderImproveReport(report) {
...
export function defaultImproveDeps() {
    return {
...
export async function runImprove(ctx, args, deps = defaultImproveDeps()) {
