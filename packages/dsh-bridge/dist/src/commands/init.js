...
function parsePackageJsonScripts(raw, source) {
    let parsed;
    try {
...
export function detectStack(io, root) {
    const notes = [];
    ...
    if (io.exists(pkgPath)) {
        ...
        if (scripts.typecheck !== undefined)
            commands.typecheck = scripts.typecheck;
        ...
        for (const key of ["build", "test", "lint", "typecheck"]) {
            ...
            if (command !== undefined)
                commands[key] = { ...command, command: command.command.replace(/^npm run /, runnerPrefix) };
        }
        return {
    ...
    if (io.exists(pyproject)) {
        ...
        return {
            ...
            ...(io.exists(`${root}/ruff.toml`) ? { lint: { label: "lint", command: "ruff check .", source: `${root}/ruff.toml` } } : {}),
            notes,
        };
    }
...
export function layoutRows(io, root) {
    return io
        .listDir(root)
...
export function renderAgentsDraft(scan, projectName) {
    const lines = [];
    lines.push(`# ${projectName}`);
    lines.push("");
    lines.push(`${projectName} - primary language/runtime: ${scan.language}. Draft generated by /bridge-init; edit freely.`);
    lines.push("");
    ...
    if (!sawAny)
        lines.push("# No install/build/test commands were detected by the scan; none are invented.");
    lines.push("```");
    lines.push("");
    lines.push("## Testing");
    lines.push("");
    lines.push(scan.test === undefined
...
export async function runInit(ctx, args) {
    void ctx;
    const io = nodeInitIo();
    ...
    const sections = [heading("/bridge-init"), ""];
    ...
    if (existingAgentsFile) {
        sections.push("Existing AGENTS.md found: import-not-overwrite per spec. Your file is untouched;", "the draft below is offered as reference material to merge by hand.", "");
    }
    else if (existingClaudeFile) {
        sections.push("Existing CLAUDE.md found and no AGENTS.md: DSH already loads CLAUDE.md directly", "(packages/context/agent-instructions/src/config.ts:12-13), so the default is no action.", "");
    }
    ...
    if (writeRequested) {
        if (existingAgentsFile || existingClaudeFile) {
            ...
            try {
                ...
                sections.push(`Write failed: ${error.message}`, "");
            }
        }
    }
    else {
