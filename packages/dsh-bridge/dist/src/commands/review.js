...
const SEVERITIES_REVIEW = ["blocker", "major", "minor", "nit"];
...
export function classifyFile(file) {
    ...
    if (/(^|\/)(dist|build|coverage|generated)(\/|$)/i.test(file))
        return "generated";
    return "review";
}
function nodeExec() {
...
export function resolveReviewTarget(args, tokens) {
...
export function diffArgv(target) {
    switch (target.kind) {
        case "worktree":
...
export function summarizeDiff(rows, maxChangedLines = MAX_CHANGED_LINES) {
    const skipped = [];
    const reviewed = [];
    let added = 0;
    let removed = 0;
...
export function findSecretLocations(diffBody) {
    const hits = [];
    let file = "";
    let lineNumber = 0;
...
export function renderReviewPrompt(target, summary, repoConventions) {
    const scope = target.kind === "base"
    ...
    for (const file of summary.reviewedFiles.slice(0, 40))
        parts.push(`- ${file}`);
    for (const skip of summary.skipped.slice(0, 10))
        parts.push(`- (skipped) ${skip.file}: ${skip.reason}`);
    return parts.join("\n");
}
// ---------------------------------------------------------------------------
// Command runner
...
export async function runReview(ctx, args, options = {}) {
    void ctx;
    ...
    if (summary.files === 0 && summary.skipped.length === 0 && !summary.truncated) {
        const hint = target.kind === "worktree"
    ...
    const secondOpinionNote = args["second-opinion"] !== undefined || args["2"] !== undefined
...
const CONVENTION_FILES = ["AGENTS.md", "CLAUDE.md", "CONTRIBUTING.md"];
function collectConventions(exec, cwd) {
    const found = [];
