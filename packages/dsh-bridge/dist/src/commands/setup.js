...
export function setupStatePath(home) {
    return join(home, ".dsh-bridge", "setup-state.json");
}
export function nodeSetupIo() {
    return {
 ...
 */
export function loadState(io, path, now = new Date()) {
    if (!io.exists(path))
    ...
    if (parsed === null || typeof parsed !== "object")
        return freshState(now);
    const record = parsed;
    if (record["version"] !== SETUP_STATE_VERSION)
        return freshState(now);
    const step = record["step"];
    if (typeof step !== "string" || !SETUP_STEPS.includes(step))
        return freshState(now);
    ...
    return {
        ...
        skipped: Array.isArray(skipped) ? skipped.filter((entry) => typeof entry === "string") : [],
...
export function saveState(io, path, state) {
    try {
...
export function detectFamiliar(io, home) {
    const found = [];
    const claudeDir = join(home, ".claude");
    if (io.exists(claudeDir)) {
        const offers = [];
        if (io.exists(join(home, ".claude.json")) || io.exists(join(claudeDir, "settings.json")))
            offers.push("mcp");
        if (io.exists(join(claudeDir, "CLAUDE.md")))
            offers.push("memory");
    ...
    if (io.exists(codexDir)) {
        const offers = [];
        if (io.exists(join(codexDir, "config.toml")))
            offers.push("mcp");
        if (io.exists(join(codexDir, "AGENTS.md")))
            offers.push("memory");
    ...
    if (io.exists(opencodeDir)) {
        const offers = [];
        if (io.exists(join(opencodeDir, "opencode.json")) || io.exists(join(opencodeDir, "config.json")))
            offers.push("mcp");
        if (io.exists(join(opencodeDir, "AGENTS.md")))
            offers.push("memory");
...
export function readDshVersion(io, profilePackageJson) {
    if (!io.exists(profilePackageJson))
        return null;
    try {
        const parsed = JSON.parse(io.readFile(profilePackageJson));
        if (parsed === null || typeof parsed !== "object")
            return null;
        const record = parsed;
        ...
        if (deps !== null && typeof deps === "object") {
            const harness = deps["@deepseek-ai/deepseek-harness"];
...
export function collectHarnessFacts(io, ctx) {
    return {
...
export function connectedProviders(rows) {
    const seen = [];
    for (const row of rows) {
        if (row.status !== "found")
            continue;
        if (PROVIDER_PROFILES[row.provider] === undefined)
            continue;
...
export function collectHealth(io, ctx) {
...
function verdictOf(entry) {
    ...
    if (text === "")
        return "no English description in the catalog";
 ...
 */
export function recommendPlugins(entries, grades, interest, limit = 5) {
    ...
    scored.sort((a, b) => {
        const gradeRank = (grade) => (grade === null ? 1 : 0);
...
function question(prompt, defaultAnswer) {
    return [
        prompt,
        "",
...
function routeBody(step, rows) {
    ...
    const routeSentence = "A route is the line of config that tells DSH which model to call and where the key for it comes from.";
    ...
    const rowsTable = step.ctx.output.table(["PROVIDER", "SOURCE", "STATUS"], rows.filter((row) => row.status === "found").map((row) => [row.provider, row.source, row.status]));
    return [
        heading("Setup - model route"),
        progress("route"),
        "",
...
function healthBody(step, findings) {
    return [
        ...
        "",
        step.ctx.output.table(["ITEM", "STATE", "DETAIL"], findings.map((finding) => [finding.label, finding.ok ? "ready" : "not set up", finding.detail])),
...
function importBody(step) {
    ...
    const rowsTable = step.ctx.output.table(["HARNESS", "PATH", "CAN CARRY OVER"], familiar.map((entry) => [
        entry.name,
        entry.path,
...
function recommendBody(step) {
    ...
    return [
        ...
        "",
        question("What do you mostly work on? A few words is enough, for example `typescript web apps` or `python data pipelines`.", "skip"),
    ].join("\n");
}
function doneBody(step, recommendations, persisted) {
    ...
    if (recommendations.length > 0) {
        ...
        lines.push("");
        lines.push(table(["PLUGIN", "GRADE", "VERDICT", "INSTALL"], recommendations.map((item) => [item.name, gradeCell(item.grade), item.verdict, item.install])));
    ...
    if (answers["import"] !== undefined && !isSkip(answers["import"])) {
        if (answers["import"] !== "memory")
            followUps.push("`/bridge-mcp import` - preview the MCP servers to carry over.");
        if (answers["import"] !== "mcp")
            followUps.push("`/bridge-memory import` - preview the instruction files to carry over.");
    }
 ...
 * have not read.
 */
export function applyAnswer(state, answer, now) {
 ...
 */
export async function runSetup(ctx, args, options = {}) {
    ...
    const stepCtx = { ctx, io, state, harness, options };
    let markdown;
    let recommendations = [];
    switch (state.step) {
        case "welcome":
