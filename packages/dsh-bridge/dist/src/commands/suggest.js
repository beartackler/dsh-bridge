...
export function intentTokens(idea) {
    return idea
        .toLowerCase()
        .split(/[^a-z0-9_-]+/)
 ...
 */
export function matchCatalog(entries, idea) {
    ...
    return candidates.sort((a, b) => b.coverage - a.coverage || b.matchedTokens.length - a.matchedTokens.length || a.entry.name.localeCompare(b.entry.name));
}
/** Grade lookup mirroring browse: card letter joined by repo base slug. */
function gradeFor(cardsDir, entry) {
...
export function scaffoldChecklist(slug) {
    return [
...
function renderScaffoldPlan(slug, idea, closest) {
    ...
    if (closest !== undefined) {
        parts.push(`Closest existing plugin (${Math.round(closest.coverage * 100)}% intent coverage): ${closest.entry.name}.`, "It did not pass the recommendation bar; fork-or-build applies. Port ideas, not code,", "and cite upstream license and attribution in NOTICE if you reuse structure.", "");
    }
...
export async function runSuggest(ctx, args, options = {}) {
    void ctx;
    ...
    const recommendable = best !== undefined &&
    ...
    if (best !== undefined && recommendable) {
        ...
        const gapNote = best.missedTokens.length > 0
...
export function suggestSlug(idea) {
