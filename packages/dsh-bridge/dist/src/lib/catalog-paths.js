 ...
 */
export function packagedDataDir(startDir = moduleDir()) {
    let dir = startDir;
 ...
 */
export function repoCatalogDir(startDir = moduleDir()) {
    let dir = startDir;
    for (let hops = 0; hops < 8; hops += 1) {
 ...
 */
export function catalogEntry(relative, startDir) {
    const roots = catalogRoots(startDir);
    for (const origin of ["repo", "packaged"]) {
...
export function searchedPaths(startDir) {
    const packaged = startDir === undefined ? packagedDataDir() : packagedDataDir(startDir);
    const repo = startDir === undefined ? repoCatalogDir() : repoCatalogDir(startDir);
    return [
...
export function unavailableDetail(file, startDir) {
