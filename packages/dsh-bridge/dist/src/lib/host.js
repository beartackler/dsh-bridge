 ...
 * wrong one.
 */
export function profileFromBaseUrl(baseUrl, dshHome) {
    ...
    if (parent !== resolve(dshHome, PROFILES_DIR))
        return undefined;
 ...
 * (journey report 3.2).
 */
export function resolveProfile(ctx, dshHome, configured, fallback = "default") {
    const mounted = profileFromBaseUrl(ctx?.baseUrl, dshHome);
    if (mounted !== undefined)
        return { name: mounted, source: "mount" };
    if (configured !== undefined && configured.trim() !== "")
        return { name: configured.trim(), source: "config" };
...
export function readActiveRoute(ctx, agent) {
 ...
 */
export function readTokenUsage(ctx, agent) {
 ...
 */
export function readHostServices(ctx, agent, mountedFeatures) {
