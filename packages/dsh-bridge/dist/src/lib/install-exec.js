 ...
 */
export function execSeamOf(ctx) {
...
function severityOf(text) {
    ...
    for (const level of ["critical", "high", "medium", "low"]) {
        if (new RegExp(`\\b${level}\\b`).test(lowered))
            return level;
    }
    return "unrated";
}
 ...
 */
export function worstCardFindings(cardMarkdown, cardPath, limit = 2) {
    ...
    for (let i = 0; i < lines.length; i += 1) {
        ...
        if (section === "4" && line.startsWith("|")) {
            ...
            if (cells.length < 2 || first === "" || /^-+:?$/.test(first))
                continue;
            if (/^(gate|finding|gate \/ finding)$/i.test(first))
                continue;
...
export function cardProvenance(cardMarkdown, cardPath) {
    const field = (label) => {
 ...
 */
export function cardFilePath(indexPath, repoRelativeCard) {
    if (indexPath === "" || repoRelativeCard === "")
        return "";
 ...
 */
export async function stageSource(source, exec, progress, makeDir = () => mkdtempSync(join(tmpdir(), "dsh-bridge-stage-"))) {
    const dir = makeDir();
    const [scheme = "", body = ""] = splitSpecifier(source);
    let command;
    if (scheme === "github")
        command = `git clone --depth 1 https://github.com/${body}.git ${dir}`;
    else if (scheme === "npm")
        command = `npm pack ${body} --pack-destination ${dir} --silent && tar -xzf ${dir}/*.tgz -C ${dir}`;
    else if (scheme === "tgz")
        command = `tar -xzf ${body} -C ${dir}`;
    else
        return { ok: false, failure: unsupportedSource(source) };
...
function splitSpecifier(source) {
...
export function classifyFailure(phase, command, result) {
...
export function noExecSeamFailure(command) {
    return {
        kind: "no-exec-seam",
...
function unsupportedSource(source) {
    return {
...
export function notMountedFailure(profile, id, dumpOutput) {
    return {
        kind: "not-mounted",
...
function dependencyRows(manifestJson) {
    try {
 ...
 */
export async function executeInstall(input) {
    const read = input.readManifest ?? ((path) => (existsSync(path) ? readFileSync(path, "utf8") : ""));
 ...
 */
export function mountedIn(dumpOutput, id, source) {
