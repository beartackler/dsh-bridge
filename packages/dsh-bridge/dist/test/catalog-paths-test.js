...
import { catalogEntry, catalogRoots, packagedDataDir, repoCatalogDir, searchedPaths, unavailableDetail, } from "../src/lib/catalog-paths.js";
...
function withPackaged(root, manifest) {
...
function withRepo(root, manifest) {
...
describe("catalog resolution order", () => {
    it("uses packaged data when no checkout exists", () => {
        ...
        assert.deepEqual(catalogRoots(moduleDir).map((entry) => entry.origin), ["packaged"]);
    });
    it("uses the repo checkout when the package ships no data", () => {
        ...
        assert.deepEqual(catalogRoots(moduleDir).map((entry) => entry.origin), ["repo"]);
    });
    it("lets a repo checkout override packaged data so live edits win", () => {
        const root = scratch();
        const moduleDir = withPackaged(root, PACKAGED);
        withRepo(root, REPO);
        assert.deepEqual(catalogRoots(moduleDir).map((entry) => entry.origin), ["packaged", "repo"], "packaged is probed first, then the checkout override");
        assert.equal(loadManifest(catalogEntry("manifest.json", moduleDir)).length, 2, "the checkout copy must win when both exist");
    ...
    it("falls back to the packaged copy for files the checkout lacks", () => {
        ...
        assert.equal(loadManifest(catalogEntry("manifest.json", moduleDir)).length, 1, "manifest.json comes from the packaged copy");
    });
    it("reports nothing and names both paths when neither source exists", () => {
        ...
        for (const path of searchedPaths(moduleDir))
...
//# sourceMappingURL=catalog-paths-test.js.map