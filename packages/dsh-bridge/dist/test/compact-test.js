...
const { DEFAULT_THRESHOLD_RATIO, MAX_INSTRUCTIONS_CHARS, formatTokens, parseCompactMode, preservedSections, rawInputFromArgs, renderStatus, runCompact, } = await import(`${dist}/commands/compact.js`);
...
describe("compact with a native hook", () => {
    it("calls compactNow exactly once and reports before/after figures", async () => {
        ...
        assert.equal(result.data.shadowedItems, 3);
    });
    ...
    it("explains why nothing was compactable, and points at status", async () => {
        ...
        assert.equal(result.data.compacted, false);
    });
...
describe("compact without a native hook", () => {
    it("emits instructions and never claims a compaction happened", async () => {
        ...
        assert.equal(result.data.hook, "absent");
    });
