...
import { applyRoute, isAppendableSequence, planRoute, renderRouteDiff, routeAlreadyPresent, routeBlock, runConnectApply, } from "../src/commands/connect-apply.js";
...
describe("refusal on an unparseable file", () => {
    ...
    it("detects an existing route in either row shape", () => {
        ...
        assert.equal(routeAlreadyPresent("- id: llm-pi-ai\n  config:\n    providers:\n      openai:\n", planRoute("openai")), true);
        assert.equal(routeAlreadyPresent("- id: llm-pi-ai\n  config:\n    providers:\n      openai:\n", planRoute("google")), false);
    });
});
describe("rollback on write failure", () => {
    ...
    it("rolls back when verification cannot find the route", () => {
        const before = "- id: existing\n";
        const files = new Map([[PATCH_PATH, before]]);
...
describe("env-ref-not-secret invariant", () => {
    const planted = { ANTHROPIC_API_KEY: PLANTED_SECRET };
...
//# sourceMappingURL=connect-apply-test.js.map