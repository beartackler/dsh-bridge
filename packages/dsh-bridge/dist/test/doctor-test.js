...
const { collectDoctorChecks, MIN_NODE_MAJOR, renderDoctorReport, runDoctor, summarizeDoctorChecks } = await import(`${dist}/commands/doctor.js`);
...
function contextOverFakeHome(home, profile = "web") {
...
function seedProfile(home, name, patchBody = "{}\n") {
    const dshHome = join(home, ".dsh");
...
describe("doctor checks on a fake HOME", () => {
    it("reports red/yellow with fix hints when every path is missing, and does not crash", () => {
        ...
        assert.deepEqual(checks.map((check) => check.id), ["node", "credentials", "profiles", "routes"]);
    ...
    it("turns fully green once profiles, config, and one well-shaped credential exist", async () => {
        ...
        writeFileSync(join(claudeDir, ".credentials.json"), JSON.stringify({ claudeAiOauth: { accessToken: "SECRET-MARKER-1234" } }), "utf8");
        ...
        const credentials = checks.find((check) => check.id === "credentials");
    ...
    it("flags malformed credential shapes as yellow while keeping other checks green", () => {
        ...
        const routes = checks.find((check) => check.id === "routes");
    ...
    it("goes yellow, not red, when the active profile dir is absent among others", () => {
        ...
        const profiles = checks.find((check) => check.id === "profiles");
    ...
    it("classifies old node runtimes red and unparseable versions yellow", () => {
        ...
        const nodeOld = old.find((check) => check.id === "node");
        ...
        const nodeWeird = weird.find((check) => check.id === "node");
...
describe("doctor aggregation and rendering", () => {
    function fixtureChecks(statuses) {
...
describe("/bridge-doctor command wiring", () => {
    ...
    it("renders the same checklist through the registry-mounted row", async () => {
        ...
        seedProfile(home, "web");
        const ctx = contextOverFakeHome(home, "web");
        const command = bridgeCommandTable(ctx).find((candidate) => candidate.name === "bridge-doctor");
