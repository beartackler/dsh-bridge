...
const { analyzeFile, auditTargets, DEFAULT_LIMIT, ImproveError, parseImproveArgs, rankFindings, renderImproveReport, resolveTargets, runImprove, } = await import(`${dist}/commands/improve.js`);
...
function analyze(name) {
    return analyzeFile(`src/${name}.ts`, fixture(name));
}
...
function memoryDeps(files, changed = []) {
    const dirs = new Set();
    ...
    return {
        ...
        readDir: (path) => Object.keys(files)
...
describe("improve detectors", () => {
    ...
    it("does not mistake a control block for a function (self-audit regression)", () => {
        ...
        const found = analyzeFile("src/x.ts", source).filter((f) => f.detector === "long-function");
        assert.equal(found.length, 1, "only the real function is reported, not each if-block");
        assert.match(found[0].cut, /small/);
    });
    it("ignores a TODO marker inside a string literal (self-audit regression)", () => {
        ...
        const found = analyzeFile("src/x.ts", source).filter((f) => f.detector === "todo-debt");
...
describe("improve ranking", () => {
    ...
    it("orders by value, then removable lines, then location", () => {
        const ranked = rankFindings(sample);
        assert.deepEqual(ranked.map((f) => `${f.path}:${f.line}`), ["a.ts:9", "b.ts:1", "a.ts:4", "b.ts:2"]);
    });
...
describe("improve target resolution", () => {
    ...
    it("audits supported files in a directory and states why others were skipped", () => {
        const resolved = resolveTargets(deps, "src");
    ...
    it("reports a missing path without throwing", () => {
        const resolved = resolveTargets(deps, "src/nope.ts");
        assert.equal(resolved.skipped[0].reason, "not found");
    });
});
describe("improve audit", () => {
    it("skips empty files and records audited line counts", () => {
        ...
        const report = auditTargets(deps, parseImproveArgs({ target: "src" }), "/repo");
        assert.deepEqual(report.skipped, [{ path: "src/empty.ts", reason: "empty" }]);
        assert.equal(report.audited.length, 1);
        assert.ok(report.audited[0].lines > 0);
    });
    it("filters by --min-value", () => {
        const deps = memoryDeps({ "src/a.ts": fixture("todo-debt") });
        const all = auditTargets(deps, parseImproveArgs({ target: "src/a.ts" }), "/repo");
        const high = auditTargets(deps, parseImproveArgs({ target: "src/a.ts", "min-value": "high" }), "/repo");
    ...
    it("truncates at --limit and reports the remainder", () => {
        ...
        const report = auditTargets(deps, parseImproveArgs({ target: "src/a.ts", limit: "1" }), "/repo");
        assert.equal(report.findings.length, 1);
        assert.equal(report.truncated, 1);
    });
    it("errors when given neither a path nor --diff", () => {
        const deps = memoryDeps({});
        assert.throws(() => auditTargets(deps, parseImproveArgs({}), "/repo"), (error) => error instanceof ImproveError && /needs a path or --diff/.test(error.message));
    });
    it("--diff audits the changed file list and filters by target prefix", () => {
        const deps = memoryDeps({ "src/a.ts": fixture("todo-debt"), "docs/x.ts": fixture("todo-debt") }, ["src/a.ts", "docs/x.ts", "pnpm-lock.yaml"]);
        const all = auditTargets(deps, parseImproveArgs({ diff: "true" }), "/repo");
        ...
        const scoped = auditTargets(deps, parseImproveArgs({ diff: "true", target: "src" }), "/repo");
...
describe("improve rendering", () => {
    ...
    it("renders one row per finding with location, detector, cut and replacement", () => {
        const deps = memoryDeps({ "src/a.ts": fixture("todo-debt") });
        const report = auditTargets(deps, parseImproveArgs({ target: "src/a.ts" }), "/repo");
        const markdown = renderImproveReport(report);
...
describe("runImprove", () => {
    it("returns markdown plus a totals payload", async () => {
        const deps = memoryDeps({ "src/a.ts": fixture("todo-debt") });
        const result = (await runImprove(ctx, { target: "src/a.ts" }, deps));
    ...
    it("never runs git except for read-only name-only diffs", async () => {
        ...
        assert.match(calls[0], /^git diff --name-only /);
    });
});
//# sourceMappingURL=improve-test.js.map