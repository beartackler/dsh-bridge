...
const { consentFor, editDistance, installCommand, loadCandidates, parseIndexGrades, repoBase, resolve, runInstall, shortId, uninstallCommand, } = installModule;
...
function candidates() {
    return loadCandidates(CATALOG.manifestPath, CATALOG.indexPath);
}
...
describe("install catalog parsing", () => {
    ...
    it("carries verdict, date, and card path onto candidates", () => {
        ...
        assert.equal(modlens.card, "docs/catalog/cards/modlens.md");
        assert.equal(modlens.verifiedAt, "2026-08-25");
    });
...
describe("install output", () => {
    it("registers in the command table", () => {
        const command = bridgeCommandTable(makeCtx()).find((c) => c.name === "bridge-install");
...
describe("install with an unavailable catalog", () => {
    ...
    it("a bare name with no catalog resolves to nothing rather than an invented source", async () => {
        ...
        assert.equal(result.data.kind, "not-found");
        assert.ok(!result.markdown.includes("dsh plugin --profile web add"));
    });
});
// ---------------------------------------------------------------------------
