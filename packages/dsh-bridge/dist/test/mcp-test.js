...
const { normalizeServerName, secretShaped, validateInstance, loadInstances, nodeMcpIo, runMcp, } = mcpModule;
const cleanupPaths = [];
after(() => {
...
async function mcpRun(args, configPath) {
...
describe("mcp list", () => {
    it("renders one row per instance plus the tool prefix column", async () => {
        const path = scratchFile(JSON.stringify({
            servers: [
                {
                    id: "mcp-github",
                    name: "@deepseek-ai/dsh-mcp-client",
