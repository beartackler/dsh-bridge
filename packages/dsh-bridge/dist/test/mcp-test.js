...
const { normalizeServerName, secretShaped, validateInstance, loadInstances, nodeMcpIo, runMcp, mcpStorePath, detectPatchEntries, } = mcpModule;
const cleanupPaths = [];
after(() => {
    for (const path of cleanupPaths)
        rmSync(path, { recursive: true, force: true });
});
function scratchFile(content) {
...
async function mcpRun(args, options) {
    const home = mkdtempSync(join(tmpdir(), "dshb-mcp-home-"));
    cleanupPaths.push(home);
    const full = {
        home,
...
describe("mcp instance validation", () => {
    it("accepts a schema-shaped stdio entry and rejects bad names/transports", () => {
        ...
        const badTransport = { ...entry, config: { ...entry.config, transport: "sse" } };
        assert.match(String(validateInstance(badTransport)), /transport must be/);
        const missingCommand = { ...entry, config: { serverName: "x", transport: "stdio" } };
...
describe("mcp config store", () => {
    ...
    it("round-trips instances through write + load", () => {
        ...
        assert.equal((loaded[0]?.config).serverName, "github");
    });
});
describe("mcp list", () => {
    it("renders one row per instance plus the tool prefix column", async () => {
        const { home } = scratchHome();
        writeFileSync(mcpStorePath(home), JSON.stringify({
            servers: [
                {
                    id: "mcp-github",
                    name: "@deepseek-ai/dsh-mcp-client",
...
describe("mcp add", () => {
    it("writes a valid stdio instance to the bridge store and prints paste instructions", async () => {
        ...
        assert.equal(loaded.length, 1);
        assert.equal((loaded[0]?.config).serverName, "gh");
    });
    ...
    it("refuses an existing serverName before any write", async () => {
        const { home, storePath } = scratchHome();
        writeFileSync(storePath, JSON.stringify({ servers: [{ id: "mcp-a", name: "@deepseek-ai/dsh-mcp-client", config: { serverName: "a", transport: "stdio", command: "x" } }] }));
        const result = await mcpRun({ _: "add a stdio other" }, { home });
        assert.match(result.markdown, /Refused/);
        const loaded = loadInstances(nodeMcpIo(), storePath);
        assert.equal(loaded.length, 1);
        assert.equal((loaded[0]?.config).command, "x");
    });
...
describe("mcp remove", () => {
    it("requires confirmation without --yes and deletes exactly one instance with it", async () => {
        ...
        assert.equal((remaining[0]?.config).serverName, "bbb");
    });
    it("unknown names exit with near-match hints and write nothing", async () => {
        const { home, storePath } = scratchHome();
        writeFileSync(storePath, JSON.stringify({ servers: [{ id: "mcp-github", name: "@deepseek-ai/dsh-mcp-client", config: { serverName: "github", transport: "stdio", command: "x" } }] }));
...
function readFileSyncForTest(path) {
