...
const { MEMORY_TEMPLATE, appendNote, datedHeading, detectImportSources, ensureMemoryFile, extractSections, importSections, memoryFilePath, memoryStatus, resolveEditor, runMemory, sectionBody, sectionHeadings, shortDigest, } = await import(`${dist}/commands/memory.js`);
    ...
    it("creates the file and names the configured editor", async () => {
        ...
        try {
            ...
            if (previous === undefined)
                delete process.env["DSH_EDITOR"];
            else
                process.env["DSH_EDITOR"] = previous;
        }
    });
...
describe("/bridge-memory add", () => {
    ...
    it("adds to an existing heading instead of duplicating it", () => {
        ...
        const content = readFileSync(memoryFilePath(home), "utf8");
        assert.equal([...sectionHeadings(content)].filter((h) => h === "Style").length, 1);
...
describe("/bridge-memory import-from", () => {
    ...
    it("detects sources and reports absent ones honestly", () => {
        ...
        const claude = sources.find((s) => s.path === join(project, "CLAUDE.md"));
        assert.ok(claude);
        assert.equal(claude.exists, true);
        assert.deepEqual([...claude.sections], ["Build", "Style"]);
        assert.ok(sources.some((s) => s.exists === false));
    });
