...
const { EXCERPT_LIMIT, PAGE_SIZE, availability, filterRows, parseResumeFilters, relativeTime, renderRows, runResume, semanticsBlock, truncateExcerpt, } = await import(`${dist}/commands/resume.js`);
...
function contextWith(sessionQuery, cwd = PROJECT) {
...
describe("resume row formatting", () => {
    ...
    it("maps availability to text badges only", () => {
        assert.equal(availability({ live: true, persisted: false }), "live");
        assert.equal(availability({ live: false, persisted: true }), "archived");
        assert.equal(availability({ live: false, persisted: false }), "unknown");
        assert.equal(availability({ live: true, persisted: true, unavailable: true }), "unavailable");
    });
...
describe("/bridge-resume command", () => {
    ...
    it("renders guidance, not an empty list, when the seam is absent", async () => {
        ...
        assert.equal(result.data.seam, "absent");
    });
