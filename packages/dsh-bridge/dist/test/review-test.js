...
const { classifyFile, diffArgv, findSecretLocations, parseNumstat, renderReviewPrompt, resolveReviewTarget, runReview, summarizeDiff, MAX_CHANGED_LINES, } = reviewModule;
const cleanupPaths = [];
after(() => {
...
describe("review numstat + classification", () => {
    ...
    it("summarizes reviewed vs skipped and enforces the changed-line budget", () => {
        ...
        assert.deepEqual(summary.skipped.map((skip) => [skip.file, skip.reason]), [
            ["pnpm-lock.yaml", "lockfile"],
            ["img.png", "binary"],
        ]);
        const overBudget = summarizeDiff(Array.from({ length: 40 }, (_, i) => ({ added: 50, removed: 0, file: `src/f${i}.ts`, binary: false })), 500);
...
describe("review renderReviewPrompt", () => {
    it("renders rubric axes in priority order with severity ladder and rules", () => {
        const prompt = renderReviewPrompt({ kind: "worktree" }, { files: 2, added: 30, removed: 4, skipped: [{ file: "pnpm-lock.yaml", reason: "lockfile" }], reviewedFiles: ["src/a.ts", "src/b.ts"], truncated: false }, ["AGENTS.md"]);
...
describe("review runner (exec double)", () => {
    ...
    it("--staged targets the cached diff and empty staged output gets its hint", async () => {
        const { fn } = fakeExec((_command, args) => {
            ...
            if (args.includes("--cached"))
                return { status: 0 }; // empty staged diff
    ...
    it("surfaces git failures as one-line errors instead of stack traces", async () => {
        const { fn } = fakeExec((_command, args) => args.includes("rev-parse") ? { status: 0 } : { status: 129, stderr: "fatal: bad revision 'nope'" });
