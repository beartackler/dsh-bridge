...
const { needsPriming, primeBlankSession, PRIMER_PLUGIN, PRIMER_TEXT } = await import(`${dist}/lib/session-priming.js`);
/** A `Session` double: append-only log, surface marker enforced on append. */
function sessionDouble(seed = []) {
    const log = [...seed];
    return {
        get events() {
            return log;
...
describe("session priming (zero-turn render workaround)", () => {
    ...
    it("treats an unmounted or unusable session as nothing to do", () => {
        ...
        assert.equal(needsPriming({ events: [] }), false);
        // A session whose events are not readable as a log.
        assert.equal(needsPriming({ events: undefined, append: () => undefined }), false);
    });
    it("never fails the command when the append is rejected", () => {
        ...
        assert.equal(primeBlankSession(rejecting), false);
    });
});
describe("command handler primes before returning output", () => {
    it("produces command output and a rendered row in a zero-turn session", async () => {
        ...
        const result = (await help.handler({ rawInput: "", agent: { session } }));
...
//# sourceMappingURL=session-priming-test.js.map