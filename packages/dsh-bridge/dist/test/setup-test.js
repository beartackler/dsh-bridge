...
const { SETUP_STEPS, applyAnswer, collectHealth, connectedProviders, detectFamiliar, isSkip, loadState, nextStep, nodeSetupIo, parseAnswer, readDshVersion, recommendPlugins, runSetup, saveState, setupStatePath, stepNumber, } = setupModule;
const cleanupPaths = [];
after(() => {
...
describe("step table", () => {
    ...
    it("recognizes every documented skip word", () => {
        for (const word of ["skip", "SKIP", " later ", "no thanks"])
            assert.equal(isSkip(word), true);
...
describe("state persistence", () => {
    it("writes state to ~/.dsh-bridge/setup-state.json and reads it back", () => {
        ...
        const state = { version: 1, step: "route", answers: { harness: "migrant" }, skipped: ["welcome"], startedAt: now.toISOString(), updatedAt: now.toISOString() };
...
describe("model route", () => {
    ...
    it("selects only known providers from the matrix", () => {
        const rows = [
            { provider: "deepseek", source: "$DEEPSEEK_API_KEY", status: "found", detail: "-" },
            { provider: "deepseek", source: "dup", status: "found", detail: "-" },
            { provider: "anthropic", source: "x", status: "expired", detail: "-" },
            { provider: "any", source: "~/.dsh/.env", status: "found", detail: "-" },
        ];
...
describe("health and import", () => {
    ...
    it("names the import commands when a familiar harness exists", async () => {
        const home = "/home/u";
        const path = setupStatePath(home);
        const io = memIo({ [path]: JSON.stringify({ version: 1, step: "import", answers: { harness: "migrant" }, skipped: [], startedAt: "x", updatedAt: "x" }), [`${home}/.claude/CLAUDE.md`]: "#" }, [`${home}/.claude`]);
        const result = await runSetup(makeCtx(home), {}, { io, env: EMPTY_ENV });
...
describe("output hygiene", () => {
    it("contains no emoji anywhere in the flow", async () => {
        ...
        for (const body of bodies)
            assert.equal(emoji.test(body), false, body.slice(0, 80));
