...
const { intentTokens, matchCatalog, runSuggest, suggestSlug, RECOMMEND_COVERAGE, } = suggestModule;
const cleanupPaths = [];
after(() => {
    for (const path of cleanupPaths)
        rmSync(path, { recursive: true, force: true });
});
function scratchCatalog(entries, cards = {}) {
    ...
    if (Object.keys(cards).length > 0) {
        ...
        for (const [name, text] of Object.entries(cards))
            writeFileSync(join(cardsDir, name), text);
...
function entry(name, repo, description) {
...
describe("suggest recommend branch", () => {
    it("recommends a grade-B high-coverage match with install hint and named gap", async () => {
        const catalog = scratchCatalog([entry("linear-sync", "acme/linear-sync", "fetch Linear tickets and sync them")], { "linear-sync.md": "| **Grade** | **B** (adjudicated) |\nhttps://github.com/acme/linear-sync\n" });
        ...
        assert.equal(result.data.mode, "recommend");
    });
    it("does not recommend a grade-D match even at high coverage; fork-or-build applies", async () => {
        const catalog = scratchCatalog([entry("linear-sync", "acme/linear-sync", "fetch Linear tickets and sync them")], { "linear-sync.md": "| **Grade** | **D** (unreviewed risk) |\nhttps://github.com/acme/linear-sync\n" });
        const result = await runSuggest(makeCtx(), { _: "fetch linear tickets and sync" }, catalog);
        assert.notEqual(result.data.mode, "recommend");
    ...
    it("never recommends below the coverage bar even with a great grade", async () => {
        const catalog = scratchCatalog([entry("gh-tools", "b/gh-tools", "github helpers")], { "gh-tools.md": "| Grade | **A** |\nhttps://github.com/b/gh-tools\n" });
