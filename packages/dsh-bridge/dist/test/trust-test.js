...
async function trustRun(args) {
    const command = bridgeCommandTable(makeCtx()).find((c) => c.name === "bridge-trust");
    assert.ok(command, "bridge-trust must be registered in the command table");
    return command.run(makeCtx(), args);
}
function makeCtx() {
...
describe("trust <plugin> card rendering", () => {
    ...
    it("shows the committed modlens card end to end when present locally", async () => {
        ...
        if (!readdirSyncSafe(cardsDir).includes("modlens.md"))
            return; // card not committed here yet
...
function mockScanReport(target, findings) {
...
function mockFinding(severity, ruleId, path) {
    return {
        id: `${ruleId}-1`,
        ruleId,
...
describe("trust scan (scan-client mock)", () => {
    ...
    it("orders mixed-severity findings worst-first with badges", () => {
        const report = parseScanReport(mockScanReport("dirty-target", [mockFinding("low", "NET-001", "a.js"), mockFinding("critical", "EXEC-001", "b.js")]));
...
describe("trust scan against temp dirs (real scanner boundary)", () => {
    ...
    it("surfaces a finding for a file containing eval()", { timeout: 60_000 }, async (t) => {
        ...
        writeFileSync(join(dir, "sneaky.js"), "const handler = (input) => eval(input);\nmodule.exports = { handler };\n", "utf8");
        const outcome = await scanDirectory(dir, { entryPath: scannerEntry, timeoutMs: 30_000 });
