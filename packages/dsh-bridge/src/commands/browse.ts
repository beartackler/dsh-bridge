 ...
 */
export function resolveGrade(entry: CatalogEntry, grades: ResolvedGrades): string | null {
...
export function loadGrades(
  ...
  const fromCards = cardsDir === undefined ? new Map<string, string>() : loadCardGrades(cardsDir, knownBases);
...
export function fuzzyScore(needle: string, haystack: string): number {
  ...
  for (let i = 0; i < lowerNeedle.length; i += 1) {
    ...
    if (found === -1) return -1;
    if (found === lastMatchIndex + 1) score += 4; // consecutive-run bonus
    else score -= found - hayIndex; // gap penalty over skipped text
    if (found === 0 || /[^a-z0-9]/.test(lowerHaystack[found - 1] ?? "")) score += 6; // boundary bonus
...
export function entryRelevance(query: string, entry: CatalogEntry): number {
...
// ---------------------------------------------------------------------------
// Filtering, ranking, pagination (pure functions)
// ---------------------------------------------------------------------------
...
export function meetsFloor(grade: string | null, floor: string): boolean {
  if (grade === null || !GRADE_FLOORS.includes(floor.toUpperCase())) return false;
...
export function passesLang(entry: CatalogEntry, lang: string | undefined): boolean {
  if (lang === "zh") return entry.descriptionZh !== "";
 ...
 */
export function sortEntries(entries: readonly CatalogEntry[], grades?: ResolvedGrades): CatalogEntry[] {
  const pointsOf = (entry: CatalogEntry): number =>
    grades === undefined ? 0 : (GRADE_POINTS[resolveGrade(entry, grades) ?? ""] ?? -1);
 ...
 */
export function filterEntries(entries: readonly CatalogEntry[], filter: BrowseFilter, grades?: ResolvedGrades): CatalogEntry[] {
  ...
  if (filter.minGrade !== undefined) {
    ...
    result = result.filter((entry) => {
      ...
      if (filter.includeUngraded && grade === null) return true;
...
function parseLang(raw: string): string | { error: string } {
...
function parseMinGrade(raw: string): string | { error: string } {
  const value = raw.toUpperCase();
  if (GRADE_FLOORS.includes(value)) return value;
...
function parseBoundedInt(raw: string, minimum: number, maximum: number): number | { error: string } {
...
export async function runBrowse(
  ...
  if (minGradeRaw !== "") {
    ...
    if (typeof parsed === "string") minGrade = parsed;
    else flagErrors.push(parsed.error);
  }
  const langRaw = args["lang"] ?? "";
  let lang: string | undefined;
  if (langRaw !== "") {
    const parsed = parseLang(langRaw);
    if (typeof parsed === "string") lang = parsed;
    else flagErrors.push(parsed.error);
  }
  ...
  if (categoryFlag !== "") filter = { ...filter, category: categoryFlag };
  if (lang !== undefined) filter = { ...filter, lang };
  if (minGrade !== undefined) filter = { ...filter, minGrade, includeUngraded };
  ...
  if (filter.category !== undefined) activeFilters.push(`category=${filter.category}`);
  if (filter.lang !== undefined) activeFilters.push(`lang=${filter.lang}`);
  if (filter.minGrade !== undefined) {
  ...
  const sections: string[] = ["### /bridge-browse", "", `${scope} | page ${page}/${pages}`, ""];
  ...
  sections.push(
    `paging: /bridge-browse${filter.category !== undefined ? ` ${filter.category}` : ""} next | prev | <page>` +
