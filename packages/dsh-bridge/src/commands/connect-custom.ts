 ...
 */
export function assertNotSecret(apiKeyEnv: string): void {
  if (SECRET_SHAPES.some((shape) => shape.test(apiKeyEnv))) {
    throw new Error(
      "--key-env takes the NAME of a credential reference, not the key itself. " +
...
export function routeKeyFromUrl(baseUrl: string): string {
  let host: string;
  try {
...
export function defaultKeyEnv(route: string): string {
 ...
 */
export function resolveCustomRoute(request: CustomRouteRequest): ResolvedCustomRoute {
  ...
  const apiKeyEnv = (request.apiKeyEnv ?? "").trim() === "" ? defaultKeyEnv(route) : (request.apiKeyEnv as string).trim();
  assertNotSecret(apiKeyEnv);

  const contextWindow = request.contextWindow ?? DEFAULT_CONTEXT_WINDOW;
  const maxTokens = request.maxTokens ?? DEFAULT_MAX_TOKENS;
  if (!Number.isInteger(contextWindow) || contextWindow <= 0) throw new Error("--context must be a positive integer.");
  ...
  return {
    ...
    baseUrl,
    model,
    displayName: (request.displayName ?? "").trim() === "" ? route : (request.displayName as string).trim(),
    modelName: (request.modelName ?? "").trim() === "" ? model : (request.modelName as string).trim(),
    apiKeyEnv,
    api: (request.api ?? "").trim() === "" ? DEFAULT_API : (request.api as string).trim(),
    contextWindow,
    maxTokens,
  };
}
 ...
 */
export function renderCustomRouteLines(route: ResolvedCustomRoute): readonly string[] {
  return [
    "- id: llm-pi-ai",
    "  config:",
    "    providers:",
...
export function renderCustomRouteYaml(route: ResolvedCustomRoute): string {
 ...
 */
export function planCustomRoute(request: CustomRouteRequest): RoutePlan {
 ...
 */
export function customApplyCommand(route: ResolvedCustomRoute): string {
  return [
