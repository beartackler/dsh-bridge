...
// ---------------------------------------------------------------------------
// State: what the previous open saw
// ---------------------------------------------------------------------------
...
export const EMPTY_DAILY_STATE: DailyState = Object.freeze({
  version: 1,
  lastOpenedOn: "",
...
export function dailyStatePath(home: string): string {
 ...
 */
export function loadDailyState(path: string): DailyState {
  let parsed: unknown;
  try {
  ...
  if (typeof parsed !== "object" || parsed === null) return EMPTY_DAILY_STATE;
...
export function saveDailyState(path: string, state: DailyState): void {
 ...
 */
export function pickRotation(entries: readonly DriftEntry[], today: string): RotationChore | null {
  ...
  const aged = candidates
    ...
    .filter((row): row is { entry: DriftEntry; age: number } => row.age !== null)
...
export function buildBriefing(
  ...
  return {
    ...
    daysSinceLastOpen:
      previous.lastOpenedOn === "" ? null : daysBetweenIso(previous.lastOpenedOn, today),
...
function attestationLines(briefing: Briefing): string[] {
  ...
  if (counts.neverAudited > 0) parts.push(`${counts.neverAudited} never audited`);
...
function choreLines(briefing: Briefing): string[] {
...
function coverageLines(briefing: Briefing): string[] {
  const { counts } = briefing;
  if (counts.tracked === 0 || counts.neverAudited === 0) return [];
  return [
...
export function renderDaily(briefing: Briefing): string {
  const blocks: string[][] = [
...
export async function runDaily(
  ...
  const entries =
    deps.drift ?? installedDrift(ctx.paths.home, ctx.paths.dshHome, ctx.profile);
