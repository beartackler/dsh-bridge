...
const USAGE =
  "Usage: /bridge-refactor <target> [plan.json] [--apply] [--rename <from>:<to>]";
...
// ---------------------------------------------------------------------------
// Exec seam (optional structural member of the context, probed at call time)
// ---------------------------------------------------------------------------
...
function getExecSeam(ctx: BridgeContext): ExecSeam | null {
  const candidate = (ctx as unknown as { readonly exec?: unknown }).exec;
...
export function collectSourceFiles(target: string): readonly string[] {
  const out: string[] = [];
...
const DECL_EXPORT_RE =
  /^export\s+(?:declare\s+)?(?:default\s+)?(?:async\s+)?(?:function\*?|class|interface|enum|const|let|var|type)\s+([A-Za-z_$][\w$]*)/;
const NAMED_LIST_RE = /^export\s*\{([^}]*)\}/;
const STAR_RE = /^export\s+(?:type\s*)?\*/;
const TOP_LEVEL_NAME_RE =
  /^(?:export\s+)?(?:declare\s+)?(?:default\s+)?(?:async\s+)?(?:abstract\s+)?(?:function\*?|class|interface|enum|const|let|var|type)\s+([A-Za-z_$][\w$]*)/;
...
export function scanSource(content: string): ScannedSource {
  ...
  for (const line of lines) {
    ...
    for (const requireMatch of line.matchAll(REQUIRE_RE)) imports.add(requireMatch[1]);
    ...
    if (STAR_RE.test(line)) exports.add("*");
  }
...
function topLevelNames(content: string): readonly string[] {
...
function declarationExportNames(content: string): readonly string[] {
  const names: string[] = [];
  for (const line of content.split("\n")) {
 ...
 */
function locateBlockEnd(lines: readonly string[], start: number): number | null {
  let depth = 0;
  for (let i = start; i < lines.length; i += 1) {
    ...
    if (depth === 0 && (trimmed.endsWith(";") || trimmed.endsWith("}"))) return i;
  }
  return null;
}
...
function referencesSibling(
  blockText: string,
  siblings: ReadonlySet<string>,
): string | null {
  for (const name of siblings) {
    if (new RegExp(`\\b${escapeRegExp(name)}\\b`).test(blockText)) return name;
  }
  return null;
}

/** Collapse the blank-line piles left behind by block removal. */
function tidyBlankLines(lines: readonly string[]): string {
  return lines.join("\n").replace(/\n{3,}/g, "\n\n");
}

// ---------------------------------------------------------------------------
// Planner (phase 2) - computed over a virtual copy of the tree
// ---------------------------------------------------------------------------
...
function findHelperCandidate(content: string): HelperCandidate | null {
  const lines = content.split("\n");
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (callHits === null || callHits.length !== 1) continue;
...
export function buildRefactorPlan(
  ...
  options: PlanOptions = {},
): { steps: RefactorStep[]; notes: string[] } {
  ...
  for (const content of contents.values()) {
    for (const name of scanSource(content).exports) publicNames.add(name);
  }
  ...
  while (cursor < worklist.length) {
    ...
    const isSplit =
      scanned.lineCount >= SPLIT_MIN_LINES && realExports.length >= 2 && !hasReExportLine;
    const isExtract =
      !isSplit &&
      scanned.lineCount < SPLIT_MIN_LINES &&
      realExports.length >= EXPORT_MODULE_MIN_EXPORTS &&
      !hasReExportLine;
    if (!isSplit && !isExtract) continue;
    ...
    for (const name of candidates) {
      ...
      const surfaceIntact =
        scanSource(newOrigin).exports.includes(name) && scanSource(moduleText).exports.includes(name);
      ...
      const kind: RefactorStepKind = isSplit ? "split-file" : "extract-module";
      steps.push({
        id: `S${steps.length + 1}`,
        kind,
  ...
  while (steps.length < MAX_PLAN_STEPS) {
    let found: { readonly path: string; readonly candidate: HelperCandidate } | null = null;
    ...
    const finalLines = withoutDecl.map((line) => {
      if (replaced || !callRe.test(line)) return line;
      replaced = true;
    ...
    const displayBody =
      candidate.body.length > 48 ? `${candidate.body.slice(0, 45)}...` : candidate.body;
    steps.push({
      id: `S${steps.length + 1}`,
      kind: "inline-helper",
  ...
  if (options.rename !== undefined) {
    if (steps.length >= MAX_PLAN_STEPS) {
      ...
      if (occurrences === 0) {
        ...
        for (const edit of edits) virtual.set(edit.path, edit.content);
      }
    }
  }

  if (truncated) notes.push(`Plan truncated at ${MAX_PLAN_STEPS} steps; remaining candidates were dropped.`);
  return { steps, notes };
}
...
export function inventoryTarget(target: string): TargetInventory {
...
export function loadPlanFile(planPath: string, target: string): RefactorPlan {
  let parsed: unknown;
  try {
  ...
  if (!Array.isArray(stepsRaw)) throw new RefactorError(`Plan file has no steps array: ${planPath}`);

  const steps: RefactorStep[] = stepsRaw.map((raw, index) => {
    const step = raw as Partial<RefactorStep> | null;
    if (
    ...
    const edits = step.edits.map((edit) => {
      const candidate = edit as Partial<RefactorEdit> | null;
    ...
    if (edits.length === 0) throw new RefactorError(`Plan step ${step.id} has no edits: ${planPath}`);
    return {
      id: step.id,
      kind: "split-file",
      title: step.title,
    ...
    for (const edit of step.edits) {
      ...
      if (resolved !== target && !resolved.startsWith(target + sep)) {
        throw new RefactorError(
          `Step ${step.id} writes outside the target path; refused before any write: ${edit.path}`,
        );
      }
    }
  }
...
// ---------------------------------------------------------------------------
// Apply (phase 3) - snapshot, step, test, roll back on red
// ---------------------------------------------------------------------------
...
function rollback(snapshot: ReadonlyMap<string, string>, written: ReadonlySet<string>): void {
...
export async function applyPlan(
  ...
  const applied: AppliedStepRecord[] = [];
...
const SAFETY_NOTES: readonly string[] = [
...
function renderPlanOnly(
  ...
  steps: readonly RefactorStep[],
  notes: readonly string[],
  loadedFrom: string | null,
): string {
  const parts: string[] = [heading("/bridge-refactor")];
  ...
  if (loadedFrom !== null) parts.push(`Steps loaded from plan file: ${loadedFrom}`);
...
function renderApply(target: string, report: ApplyReport): string {
  const parts: string[] = [
  ...
  if (report.rolledBack) {
    parts.push(
      `ROLLED BACK at ${report.failedStepId}: ${TEST_COMMAND} exited ${report.testExitCode}; every write was restored from the pre-apply snapshot.`,
    );
...
function parseRename(raw: string): { from: string; to: string } | { error: string } {
  const idx = raw.indexOf(":");
  if (idx < 0) return { error: '--rename expects <from>:<to> with plain identifier names.' };
...
export async function runRefactor(
  ...
  if (targetArg === "") return usageResult();
  const target = resolve(targetArg);
  if (!existsSync(target)) return fail(`No such file or directory: ${target}`);
  ...
  try {
    ...
    if (inventory.files.length === 0) {
      return fail(
        `No source files under ${target}. Supported extensions: ${[...SOURCE_EXTENSIONS].join(" ")}`,
      );
    }
    ...
    if (error instanceof RefactorError) return fail(error.message);
    throw error;
  }
}
