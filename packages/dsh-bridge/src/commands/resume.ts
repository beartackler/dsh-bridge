...
export function sessionQueryHooks(ctx: BridgeContext): SessionQueryHooks {
  return (ctx as ResumeContext).sessionQuery ?? {};
}
...
export function filterRows(
  ...
  return rows.filter((row) => {
    if (!filters.subagents && row.origin === "subagent") return false;
    if (!filters.all && row.cwd !== undefined && row.cwd !== filters.cwd) return false;
...
export function availability(row: SessionRow): string {
  if (row.unavailable === true) return "unavailable";
  if (row.live) return "live";
  if (row.persisted) return "archived";
  return "unknown";
}
...
export function semanticsBlock(): string {
  return [
    "Resume vs fork:",
    "",
    bulletList([
...
export function renderRows(rows: readonly SessionRow[], filters: ResumeFilters, now: number): string {
...
export function renderEmpty(filters: ResumeFilters): string {
  return [
    heading("/bridge-resume"),
    filters.all
 ...
 */
export function renderNoSeam(): string {
  return [
    heading("/bridge-resume"),
...
export async function runResume(
  ...
  const cwd = (ctx as ResumeContext).cwd ?? process.cwd();
  ...
  const parts: string[] = [rows.length === 0 ? renderEmpty(filters) : renderRows(rows, filters, now)];
