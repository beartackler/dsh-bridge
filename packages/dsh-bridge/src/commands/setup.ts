...
import { applyRoute, credentialInstructions, nodeApplyIo, routeBlock, type ApplyIo, type ApplyOutcome, type RoutePlan } from "./connect-apply.js";
...
// ---------------------------------------------------------------------------
// The route step's write (the thing between "installed" and "answering")
// ---------------------------------------------------------------------------
 ...
 */
export function isRouteWrite(args: Readonly<Record<string, string>>): boolean {
 ...
 */
export function writeRouteFromArgs(
  ctx: BridgeContext,
  args: Readonly<Record<string, string>>,
  io: ApplyIo,
): RouteWriteResult | { readonly error: string } {
  let plan: RoutePlan;
  try {
...
export function routeWrittenBody(step: StepContext, result: RouteWriteResult): string {
 ...
 */
function customRouteOffer(step: StepContext): readonly string[] {
  return [
...
export async function runSetup(
  ...
  if (state.step === "route" && isRouteWrite(args)) {
    ...
    const stepCtxNow: StepContext = { ctx, io, state, harness: harnessNow, options };
