 ...
 */
function parseCard(markdown: string): ParsedCard {
  const lines = markdown.split(/\r?\n/);
  const title = lines.find((line) => line.startsWith("# "))?.slice(2).trim() ?? "";

  function sectionBody(headerPattern: RegExp): string {
...
export function gradeFromCard(markdown: string): string | null {
...
function notReviewed(slug: string): string {
  return [
    `### Trust report: ${slug}`,
    "",
    "NOT REVIEWED",
 ...
 */
export function renderCard(ctx: BridgeContext, slug: string, markdown: string): string {
  ...
  if (parsed.headerTable !== "") parts.push(parsed.headerTable, "");
  if (parsed.verdict !== "") parts.push(`**Verdict:** ${parsed.verdict}`, "");
  parts.push(
    `Evidence: \`docs/catalog/cards/${slug}.md\`. A grade covers one pinned artifact only; other versions are unreviewed.`,
    "",
  );
  return parts.join("\n");
}

async function showCard(ctx: BridgeContext, rawName: string): Promise<string> {
  const slug = toSlug(rawName);
  if (slug === "") return ["### /bridge-trust", "", USAGE, ""].join("\n");

  const path = join(cardsDir(), `${slug}.md`);
  if (!existsSync(path)) return notReviewed(slug);
  return renderCard(ctx, slug, readFileSync(path, "utf8"));
}

function listCards(ctx: BridgeContext): string {
  const dir = cardsDir();
  let files: string[];
  try {
 ...
 */
export function renderScanSummary(ctx: BridgeContext, target: string, report: ScanReport): string {
  ...
  if (report.findings.length > 0) {
    ...
    parts.push(
      ctx.output.table(
        ...
        ordered.slice(0, 5).map((f) => [ctx.output.badge(f.severity), f.ruleId, `${f.path}:${f.line}`, f.message]),
      ),
    );
  } else {
    parts.push(
...
async function scanTarget(ctx: BridgeContext, target: string): Promise<string> {
  const resolved = target.trim();
  if (resolved === "") return ["### /bridge-trust scan", "", "Usage: /bridge-trust scan <directory>", ""].join("\n");
  const { report } = await scanDirectory(resolved);
  return renderScanSummary(ctx, resolved, report);
}

// ---------------------------------------------------------------------------
// refresh: the drift watch's action half
// ---------------------------------------------------------------------------
 ...
 */
export function renderRefresh(ctx: BridgeContext, outcomes: readonly RefreshOutcome[]): string {
  ...
  const parts: string[] = ["### Trust refresh", ""];
  parts.push(
    ctx.output.table(
      ["PLUGIN", "CARD", "LOCAL", "NEW", "RESOLVED"],
      outcomes.map((o) => [
        o.slug,
  ...
  for (const outcome of outcomes) {
    ...
    if (outcome.firstAudit) {
      parts.push(
        "First local audit. Every finding below is new to the local record, not",
        "new to the plugin; there was no earlier scan to compare against.",
        "",
      );
    }
    ...
    if (outcome.cardGrade === null) {
      parts.push(
        `No card exists for \`${outcome.slug}\`, so nothing was annotated and no`,
        "published grade is implied. The local grade above is this machine's scan",
        "of this machine's copy, and it is not a dsh-bridge review.",
        "",
        `Next: \`/bridge-trust queue ${outcome.slug}\` request a maintainer review.`,
        "",
      );
    } else if (outcome.annotated) {
      parts.push(
        `Annotated \`docs/catalog/cards/${outcome.slug}.md\` on its Audited row.`,
        "The published grade was not changed: a local scan cannot raise or lower it.",
        "",
      );
    } else {
      parts.push(
        `Card \`docs/catalog/cards/${outcome.slug}.md\` has no Audited row to annotate;`,
        "the diff above is reported without touching the card.",
        "",
      );
    }
  }
...
export async function refreshInstalled(
  ...
  subject: string,
  deps: RefreshDeps,
): Promise<{ markdown: string; outcomes: RefreshOutcome[] }> {
  const discover = deps.discover ?? discoverInstalledPlugins;
  const scan = deps.scan ?? (async (dir: string) => (await scanDirectory(dir)).report);
  ...
  for (const plugin of installed) {
    ...
    let cardGrade: string | null = null;
    let annotated = false;
  ...
  if (outcomes.length > 0) writeState(statePath, state);
  return { markdown: renderRefresh(ctx, outcomes), outcomes };
}

/** Entry point wired into lib/registry.ts as `bridge-trust`. */
export async function runTrust(ctx: BridgeContext, args: Readonly<Record<string, string>>): Promise<{ markdown: string }> {
  const positional = (args["_"] ?? "").trim();
  const rest = (args["rest"] ?? "").trim();

  if (positional === "scan") return { markdown: await scanTarget(ctx, rest) };
  if (positional === "list") return { markdown: listCards(ctx) };
