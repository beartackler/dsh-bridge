 ...
 */
const NUMERIC_HEADERS: ReadonlySet<string> = new Set([
  "#",
  "STARS",
  "LINES",
  "MESSAGES",
...
function ruleCell(header: string): string {
 ...
 */
const GRADE_BANDS: Readonly<Record<string, string>> = Object.freeze({
  A: "Verified-clean",
 ...
 */
function splitGrade(letter: string | null | undefined): { key: string; suffix: string } | null {
...
export function gradeCell(letter: string | null | undefined): string {
 ...
 */
export function gradeLabel(letter: string | null | undefined): string {
  const parsed = splitGrade(letter);
  if (parsed === null) return UNREVIEWED;
...
export function normalizeSpacing(markdown: string): string {
  ...
  for (const raw of lines) {
    ...
    if (!inFence && line === "" && out.length === 0) continue;
    out.push(line);
  }
