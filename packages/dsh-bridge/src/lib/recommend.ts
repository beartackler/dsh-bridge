...
export const WORK_TYPES = ["web", "backend", "data", "mobile", "devops", "writing"] as const;
 ...
 */
export function inferWorkType(answer: string): WorkType | undefined {
  const text = answer.toLowerCase();
  const rules: readonly (readonly [WorkType, RegExp])[] = [
...
export const BLOCKED_GRADES: readonly string[] = ["D", "F"];
 ...
 */
const CATEGORY_FIT: Readonly<Record<WorkType, { primary: readonly string[]; adjacent: readonly string[] }>> =
  Object.freeze({
 ...
 */
const DEFAULT_CATEGORIES: readonly string[] = ["dev", "tools", "session", "memory", "git"];
 ...
 */
export function parseVerifiedDates(indexMarkdown: string): Map<string, string> {
...
export function starsPoints(stars: number | null): number {
...
export function categoryPoints(category: string, work: WorkType | null): number {
  if (work === null) {
    return DEFAULT_CATEGORIES.includes(category) ? CATEGORY_PRIMARY : 0;
  }
  const fit = CATEGORY_FIT[work];
  if (fit.primary.includes(category)) return CATEGORY_PRIMARY;
  if (fit.adjacent.includes(category)) return CATEGORY_ADJACENT;
  return 0;
}
...
function reasonFor(entry: CatalogEntry, grade: string | null, work: WorkType | null, harnessHit: boolean): string {
  if (grade === null) {
  ...
  if (fit === CATEGORY_PRIMARY && work !== null) parts.push(`core ${work} tooling`);
  else if (fit === CATEGORY_ADJACENT && work !== null) parts.push(`adjacent to ${work} work`);
  else if (fit === CATEGORY_PRIMARY) parts.push("broadly useful");
  else parts.push(`${entry.category} category`);
  if (harnessHit) parts.push("eases the move from your previous harness");
  return parts.join(", ");
}

/** True when the entry's text names the harness the user is migrating from. */
function mentionsHarness(entry: CatalogEntry, priorHarness: string | undefined): boolean {
  if (priorHarness === undefined || priorHarness.trim() === "") return false;
...
function installedKeys(installed: readonly string[]): Set<string> {
...
function isInstalled(entry: CatalogEntry, keys: ReadonlySet<string>): boolean {
...
export function recommend(
  ...
  const gradeOf = options.gradeOf ?? ((): string | null => null);
  const verifiedAt = options.verifiedAt ?? new Map<string, string>();
  const now = options.now ?? new Date();
  const work = profile.work ?? null;
  const skip = installedKeys(profile.installed ?? []);

  const graded: Scored[] = [];
  const ungraded: Scored[] = [];

  for (const entry of entries) {
    if (isInstalled(entry, skip)) continue;
    ...
    if (grade !== null && BLOCKED_GRADES.includes(grade)) continue;
  ...
  const toRecommendation = (scored: Scored): Recommendation => ({
...
export const RECOMMEND_HEADERS: readonly string[] = ["GRADE", "PLUGIN", "CATEGORY", "STARS", "WHY THIS"];

export function recommendationRows(picks: readonly Recommendation[]): readonly (readonly string[])[] {
