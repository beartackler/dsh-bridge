...
async function browseRun(args: Readonly<Record<string, string>>): Promise<RunResult> {
  const command = bridgeCommandTable(makeCtx()).find((c: { name: string }) => c.name === "bridge-browse");
  assert.ok(command, "bridge-browse must be registered in the command table");
  return (await command.run(makeCtx(), args)) as Promise<RunResult>;
}
...
function writeFixture(dir: string, entries: readonly FixtureEntry[]): { manifestPath: string; cardsDir: string } {
...
function cardWith(repoLink: string, grade: string): string {
  return [
    "# Trust Report Card",
    "",
    "| | |",
 ...
 */
function fixtureCatalog(): { manifestPath: string; cardsDir: string; indexMdPath: string } {
  ...
  writeFileSync(
    join(paths.cardsDir, "memstore.md"),
...
describe("browse grade join", () => {
  it("resolves grades from INDEX by repo, then name, then falls back to cards", async () => {
    ...
    const byRepo = entries.find((e: { repo: string }) => e.repo === "acme/dsh-web-ui");
    assert.ok(byRepo);
    assert.equal(resolveGrade(byRepo!, grades), "B");

    const byNameFallback = entries.find((e: { repo: string }) => e.repo === "owner/recall");
    ...
    const withBoth = entries.find((e: { repo: string }) => e.repo === "owner/alpha-plugin");
    assert.ok(withBoth);
    assert.equal(resolveGrade(withBoth!, grades), "C");

    // memstore's audit lives only on its committed card.
    const cardOnly = entries.find((e: { repo: string }) => e.repo === "acme/memstore");
    assert.ok(cardOnly);
    assert.equal(resolveGrade(cardOnly!, grades), "A");

    const unreviewed = entries.find((e: { repo: string }) => e.repo === "owner/beta-pack");
...
describe("browse fuzzy matching", () => {
  const options = (): { manifestPath: string; cardsDir: string; indexMdPath: string } => fixtureCatalog();
  ...
  it("resolves the spec's canonical typos to their intended entries", async () => {
    ...
    const dataRow = gitgraph.markdown.split("\n").find((line: string) => line.includes("web-ui-gitgraph"));
...
describe("browse flag filters", () => {
  ...
  it("filters by --category (default lang=en already gated zh-only out)", async () => {
    ...
    const names = result.markdown.split("\n").filter((line: string) => line.includes("| owner/")).length;
  ...
  it("composes --category AND --min-grade", async () => {
    ...
    assert.ok(rows.every((row: string) => /\|\s*[ABC]\s*\|/.test(row)), "only graded rows survive");
...
describe("browse pagination boundaries", () => {
  function paged(): { manifestPath: string; cardsDir: string; indexMdPath?: undefined } {
    const dir = scratchDir("dshb-pages-");
    const paths = writeFixture(
      dir,
      Array.from({ length: 25 }, (_, index): FixtureEntry => ({
...
describe("browse ranking (grade then stars)", () => {
  it("puts graded entries above higher-starred unreviewed ones", async () => {
    ...
    const order = result.markdown
      .split("\n")
      .filter((line: string) => line.includes("| owner/"))
      .map((line: string) => (line.includes("web-ui-gitgraph") ? "graded-B" : "unreviewed"));
...
describe("browse empty state and install handoff", () => {
  ...
  it("exposes structured data and degrades gracefully without a manifest", async () => {
    ...
    const data = found.data as { mode: string; total: number; pages: number };
