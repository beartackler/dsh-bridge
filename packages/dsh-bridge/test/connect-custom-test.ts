...
import { applyRoute, routeAlreadyPresent, routeBlock, selectionPresent, type ApplyIo } from "../src/commands/connect-apply.js";
import {
  assertNotSecret,
  defaultKeyEnv,
  planCustomRoute,
  renderCustomRouteYaml,
  resolveCustomRoute,
  routeKeyFromUrl,
} from "../src/commands/connect-custom.js";
import { parseConnectArgs, parseCustomArgs, runConnect } from "../src/commands/connect.js";
import { isRouteWrite, runSetup, type SetupIo } from "../src/commands/setup.js";
...
const ZEN_ARGS: Readonly<Record<string, string>> = Object.freeze({
  _: "custom",
...
function fakeIo(
  ...
  options: { readonly dropSelection?: boolean; readonly failWrite?: boolean } = {},
): ApplyIo & { files: Map<string, string> } {
  ...
  return {
    ...
    writeFile: (path, content) => {
      if (options.failWrite === true) throw new Error("EACCES: injected write failure");
...
function fakeSetupIo(initial: Readonly<Record<string, string>> = {}): SetupIo & { files: Map<string, string> } {
  const files = new Map(Object.entries(initial));
  return {
    files,
    exists: (path) => files.has(path),
...
describe("secret safety", () => {
  it("refuses a key-shaped --key-env before anything is planned", () => {
    ...
    assert.throws(
      () => planCustomRoute({ route: "zen", baseUrl: "https://a.example/v1", model: "m", apiKeyEnv: PLANTED_SECRET }),
  ...
  it("refuses the setup route write when the key is pasted as the variable name", async () => {
    ...
    const result = await runSetup(
      makeContext(),
      { ...ZEN_ARGS, _: "", "key-env": PLANTED_SECRET },
      { io, applyIo, now: new Date("2026-02-02T00:00:00.000Z") },
    );
    ...
    assert.equal((result.data as { kind: string }).kind, "setup.route.refused");
  });
});
...
describe("command surface", () => {
  ...
  it("previews without writing and names the exact command that would write", async () => {
    ...
    assert.equal((result.data as { kind: string }).kind, "connect.apply.preview");
  });
  ...
  it("renders a usage body rather than throwing on a bad custom invocation", async () => {
    ...
    assert.equal((result.data as { kind: string }).kind, "connect.custom.refused");
  });
});
...
describe("setup completes the job", () => {
  ...
  it("writes the route, backs the file up, and reports what changed", async () => {
    ...
    const data = result.data as { kind: string; verified: boolean; backupPath?: string; envVar: string };
