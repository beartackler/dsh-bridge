...
import type { BridgeContext, DetectionRow } from "../src/lib/types.js";
...
function makeContext(home: string): BridgeContext {
...
describe("matrix rendering", () => {
  const rows: readonly DetectionRow[] = [
...
describe("argument parsing", () => {
  ...
  it("requires a provider after test", () => {
    assert.throws(() => parseConnectArgs({ _: "test" }), new RegExp(SMOKE_PROVIDERS[0] as string));
  });
});
