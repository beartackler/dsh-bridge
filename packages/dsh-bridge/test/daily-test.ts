...
function entry(
  pkg: string,
  state: "aligned" | "changed" | "never-audited",
  auditedOn: string | null = "2026-08-01",
) {
  const slug = pkg.split("/").pop() as string;
  return {
    pkg,
    slug,
    state,
...
describe("pickRotation", () => {
  ...
  it("prefers a never-audited plugin over a merely old one", () => {
    const chore = pickRotation(
      [entry("@x/old", "aligned", "2020-01-01"), entry("@x/fresh", "never-audited")],
      "2026-08-26",
    );
    assert.equal(chore?.slug, "fresh");
    assert.equal(chore?.ageDays, null);
    assert.match(chore?.reason as string, /never audited/);
  });
  ...
  it("picks the oldest audit when all are audited", () => {
    ...
    assert.match(chore?.reason as string, new RegExp(`${ROTATE_AFTER_DAYS}-day mark`));
  });

  it("excludes a drifted plugin, whose prompt is the alarm, not the chore", () => {
    const chore = pickRotation(
      [entry("@x/moved", "changed", "2020-01-01"), entry("@x/still", "aligned", "2026-08-20")],
      "2026-08-26",
    );
  ...
  it("does not claim the mark was passed when the audit is recent", () => {
    ...
    assert.doesNotMatch(chore?.reason as string, /mark/);
  });
});
...
describe("buildBriefing", () => {
  it("keeps never-audited apart from aligned", () => {
    const briefing = buildBriefing(
      [entry("@x/a", "aligned"), entry("@x/b", "never-audited"), entry("@x/c", "changed")],
      EMPTY_DAILY_STATE,
      "2026-08-26",
    );
  ...
  it("reports newly-changed as a set difference against the last open", () => {
    ...
    const briefing = buildBriefing(
      [entry("@x/known", "changed"), entry("@x/surprise", "changed")],
      previous,
      "2026-08-26",
    );
...
describe("renderDaily on an eventful day", () => {
  ...
  const markdown = renderDaily(
    buildBriefing(
      [entry("@liustack/modlens", "changed", "2026-08-01"), entry("@x/steady", "aligned", "2026-08-20")],
      previous,
      "2026-08-26",
    ),
  );
...
describe("renderDaily honesty paths", () => {
  ...
  it("counts never-audited plugins as unknown, not clean", () => {
    const markdown = renderDaily(
      buildBriefing(
        [entry("@x/a", "aligned", "2026-08-20"), entry("@x/b", "never-audited"), entry("@x/c", "never-audited")],
        EMPTY_DAILY_STATE,
        "2026-08-26",
      ),
    );
  ...
  it("says so on a first briefing rather than inventing a delta", () => {
    const markdown = renderDaily(
      buildBriefing([entry("@x/a", "aligned")], EMPTY_DAILY_STATE, "2026-08-26"),
    );
  ...
  it("distinguishes a still-drifted plugin from a newly drifted one", () => {
    const markdown = renderDaily(
      buildBriefing(
        [entry("@x/known", "changed", "2026-08-01")],
        { version: 1, lastOpenedOn: "2026-08-24", changedPkgs: ["@x/known"], trackedCount: 1 },
        "2026-08-26",
      ),
    );
  ...
  it("agrees in number when several plugins are still drifted", () => {
    const markdown = renderDaily(
      buildBriefing(
        [entry("@x/known", "changed", "2026-08-01"), entry("@x/other", "changed", "2026-08-01")],
        {
          version: 1,
  ...
  it("names an already-drifted package on a first open instead of hiding it", () => {
    const markdown = renderDaily(
      buildBriefing([entry("@x/moved", "changed", "2026-08-01")], EMPTY_DAILY_STATE, "2026-08-26"),
    );
    assert.match(markdown, /First briefing on this machine/);
...
describe("runDaily", () => {
  ...
  it("returns the briefing as structured data alongside the markdown", async () => {
    ...
    const result = await runDaily(
      makeCtx(home),
      {},
      { drift: [entry("@x/a", "aligned", "2026-08-20")], now: new Date("2026-08-26T00:00:00Z") },
    );
    const data = result.data as { date: string; counts: { tracked: number } };
