...
function makeReport(
  grade: string,
  findings: readonly { ruleId: string; path: string; line: number }[],
): unknown {
  return {
...
describe("drift hashPluginDir", () => {
  it("is stable across repeated walks of unchanged content", () => {
    ...
    assert.match(first as string, /^sha256:[0-9a-f]{64}$/);
...
describe("drift discoverInstalledPlugins", () => {
  ...
  function makeProfile(): string {
    ...
    writeFileSync(
      ...
      JSON.stringify({ dependencies: { "@liustack/modlens": "^3.25.0", "dsh-context": "^1.0.0", "ghost-dep": "^1.0.0" } }),
    );
    for (const [pkg, version] of [["@liustack/modlens", "3.25.0"], ["dsh-context", "1.0.0"]] as const) {
  ...
  it("reads dependencies and resolves each to its installed directory", () => {
    ...
    assert.deepEqual(found.map((p: { pkg: string }) => p.pkg), ["@liustack/modlens", "dsh-context"]);
  ...
  it("skips a declared dependency with no directory rather than reporting a phantom", () => {
    ...
    assert.equal(found.find((p: { pkg: string }) => p.pkg === "ghost-dep"), undefined);
  });
...
describe("drift detectDrift", () => {
  ...
  const hashes: Record<string, string> = { "/p/a": "sha256:aa", "/p/b": "sha256:zz", "/p/c": "sha256:cc" };
  const entries = detectDrift(installed, state, (dir: string) => hashes[dir] ?? null);

  it("classifies aligned, changed, and never-audited plugins", () => {
    assert.deepEqual(
      entries.map((e: { pkg: string; state: string }) => [e.pkg, e.state]),
      [["a", "aligned"], ["b", "changed"], ["c", "never-audited"]],
    );
  });
...
describe("drift annotateCardAudited", () => {
  ...
  it("appends the annotation inside the Audited cell", () => {
    ...
    const line = out.split("\n").find((l: string) => l.startsWith("| Audited")) as string;
...
describe("trust refresh", () => {
  ...
  function makeWorld(pkg = "@liustack/modlens"): { dshHome: string; home: string; cards: string; dir: string } {
  ...
  it("handles the no-card-yet path without implying a published grade", async () => {
    ...
    assert.equal(
      loadAuditState(auditStatePath(world.home)).plugins["brand-new-plugin"].hash,
      hashPluginDir(world.dir),
    );
  });
...
describe("status drift line", () => {
  ...
  it("reports drift through runStatus data for UI consumers", async () => {
    ...
    assert.deepEqual((result.data as { drift: unknown[] }).drift, entries);
  });
});
