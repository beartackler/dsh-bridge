...
const { profileFromBaseUrl, readActiveRoute, readHostServices, readTokenUsage, resolveProfile } = await import(
  `${dist}/lib/host.js`
);
...
function profileBaseUrl(name: string, dshHome = DSH_HOME): string {
 ...
 */
function ctxWith(services: Record<string, unknown>, baseUrl?: string) {
  return {
...
function agentDefaultModelDouble(selection: unknown) {
  return {
 ...
 */
function sessionProjectionsDouble(values: Record<string, unknown>, asOfSeq = 42) {
  const calls: unknown[] = [];
  return {
    calls,
    snapshot(session: unknown): unknown {
      calls.push(session);
...
function agentDouble(options: Record<string, unknown> = {}, session: unknown = { id: "sess-1" }) {
  return { id: "sess-1", options, session, status: "idle" as const };
}
...
describe("readTokenUsage", () => {
  ...
  it("reads usage without a contextPressure projection", () => {
    ...
    const usage = readTokenUsage(
      ctxWith({ sessionProjections: sessionProjectionsDouble({ tokenUsage: TOKEN_USAGE }) }),
      agentDouble(),
    );
    assert.equal((usage as { contextWindow?: number }).contextWindow, undefined);
    assert.equal((usage as { pressureTokens?: number }).pressureTokens, undefined);
    assert.equal((usage as { uncachedInputTokens: number }).uncachedInputTokens, 8500);
  });
  ...
  it("returns undefined with no projection registry and with no invoking agent", () => {
    ...
    assert.equal(
      readTokenUsage(ctxWith({ sessionProjections: sessionProjectionsDouble({ tokenUsage: TOKEN_USAGE }) }), undefined),
      undefined,
    );
  });
  ...
  it("refuses a projection whose required counters are missing or non-finite", () => {
    for (const tokenUsage of [{ outputTokens: 5 }, { uncachedInputTokens: 5 }, { uncachedInputTokens: Number.NaN, outputTokens: 5 }]) {
      assert.equal(
        readTokenUsage(ctxWith({ sessionProjections: sessionProjectionsDouble({ tokenUsage }) }), agentDouble()),
        undefined,
      );
    }
  });
...
describe("readHostServices", () => {
  ...
  it("degrades one row at a time, never cascading", () => {
    ...
    const host = readHostServices(
      ctxWith({ agentDefaultModel: agentDefaultModelDouble({ provider: "pi-ai", model: "qwen-3.5-plus" }) }),
      agentDouble(),
      ["commands"],
    );
