...
const {
  ...
  resolveRouteToken,
  runModel,
} = await import(`${dist}/commands/model.js`);
...
describe("model route collection", () => {
  it("joins registered and declared routes with derived availability", () => {
    ...
    assert.deepEqual(
      routes.map((route: { id: string }) => route.id),
      [
...
describe("model rendering", () => {
  ...
  it("announces a winning session override when one is set", () => {
    const overridden = { ...(fixtureConfig() as object), sessionOverride: { provider: "moonshot", model: "kimi-k2" } };
    const markdown = renderModelList(ctx, collectRoutes(overridden as never), overridden as never);
...
describe("model command runner", () => {
  ...
  it("stages use and test without writing any config file", async () => {
    const use = await runModel(ctx, { _: "use", rest: "deepseek/deepseek-chat" }, { config: fixtureConfig() as never });
    assert.match(use.markdown, /Staged switch/);
    assert.ok(use.data === undefined, "mutation-free verbs carry no data payload");

    const useBare = await runModel(ctx, { _: "use", rest: "deepseek-reasoner" }, { config: fixtureConfig() as never });
    assert.match(useBare.markdown, /deepseek\/deepseek-reasoner/);

    const test = await runModel(ctx, { _: "test", rest: "deepseek/deepseek-chat" }, { config: fixtureConfig() as never });
    assert.match(test.markdown, /SMOKE TEST PROCEDURE/);

    const refused = await runModel(ctx, { _: "test", rest: "kimi-k2" }, { config: fixtureConfig() as never });
  ...
  it("answers unknown routes and bad args with usage, not crashes", async () => {
    const unknown = await runModel(ctx, { _: "use", rest: "ghost/model" }, { config: fixtureConfig() as never });
    assert.match(unknown.markdown, /unknown route/);

    const badVerb = await runModel(ctx, { _: "wat" }, { config: fixtureConfig() as never });
  ...
  it("never leaks a credential value even if one somehow reached the double", async () => {
    ...
    const result = await runModel(ctx, {}, { config: poisoned as never });
  ...
  it("keeps every rendered byte ASCII and emoji-free across all verbs", async () => {
    const config = fixtureConfig() as never;
    const outputs = [
