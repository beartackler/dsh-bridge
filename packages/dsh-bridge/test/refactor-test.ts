...
const {
  ...
  SPLIT_MIN_LINES: number;
  TEST_COMMAND: string;
  applyPlan: (
    plan: unknown,
    runCommand: (request: { command: string; cwd?: string }) => Promise<{ code: number; stdout: string; stderr: string }>,
    cwd: string,
  ) => Promise<{
    applied: { stepId: string; testExitCode: number; status: "applied" | "failed" }[];
    rolledBack: boolean;
    failedStepId?: string;
    testExitCode?: number;
    stderrTail?: string;
  ...
  ) => { steps: any[]; notes: string[] };
  inventoryTarget: (target: string) => { files: any[]; contents: Map<string, string> };
  loadPlanFile: (planPath: string, target: string) => unknown;
  RefactorError: new (message: string) => Error;
  runRefactor: (
    ctx: unknown,
    args: Readonly<Record<string, string>>,
  ) => Promise<{ markdown: string; data?: unknown }>;
  scanSource: (content: string) => { lineCount: number; imports: string[]; exports: string[] };
};
...
function makeCtx(exec?: (request: { command: string }) => Promise<{ code: number; stdout: string; stderr: string }>) {
...
function scratchDir(prefix = "refactor-fixture-"): string {
...
describe("refactor plan-only default", () => {
  it("renders inventory and steps without touching any file", async () => {
    ...
    const result: RunResult = await runRefactor(makeCtx(), { _: dir });
    ...
    assert.equal((result.data as { mode?: string }).mode, "plan");
  });
  ...
  it("reports zero steps for a clean small file instead of inventing work", async () => {
    ...
    assert.equal(((result.data as { steps?: unknown[] }).steps ?? []).length, 0);
...
describe("refactor split-file with green apply", () => {
  it("plans re-exporting splits and applies them step by step", async () => {
    ...
    const result: RunResult = await runRefactor(ctx, { _: dir, apply: "" });

    assert.equal((result.data as { mode?: string }).mode, "apply");
    ...
    const appliedSteps = (result.data as { applied: unknown[] }).applied.length;
    assert.equal(calls.length, appliedSteps);
    for (const call of calls) assert.equal(call, TEST_COMMAND);
...
describe("refactor rollback on red", () => {
  it("restores the snapshot and reports the failing step", async () => {
    ...
    const ctx = makeCtx(async (request) => {
      ...
      if (runCount === 1) return { code: 0, stdout: "ok\n", stderr: "" };
...
describe("refactor safety", () => {
  it("refuses --apply without a test seam and writes nothing", async () => {
    ...
    const result: RunResult = await runRefactor(makeCtx(), { _: dir, apply: "" });
  ...
  it("refuses a plan file whose steps escape the target before any write", async () => {
    ...
    const result: RunResult = await runRefactor(makeCtx(), { _: targetDir, rest: planPath, apply: "" });
  ...
  it("rejects malformed plan files without writing", async () => {
    ...
    const result: RunResult = await runRefactor(makeCtx(), { _: targetDir, rest: badPlan, apply: "" });
  ...
  it("flags exported renames as [public] and respects word boundaries", async () => {
    ...
    const result: RunResult = await runRefactor(makeCtx(), { _: dir, rename: "userName:accountName" });
    assert.equal((result.data as { mode?: string }).mode, "plan");
    const steps = (result.data as { steps: { touchesPublicSurface: boolean; kind: string }[] }).steps;
    ...
    const ctx = makeCtx(async () => ({ code: 0, stdout: "", stderr: "" }));
    const applied: RunResult = await runRefactor(ctx, { _: dir, rename: "userName:accountName", apply: "" });
