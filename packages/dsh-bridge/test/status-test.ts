...
function makeCtx(dshHome: string, profile = "web") {
...
function indexFixture(now: Date): string {
  return [
    "# Catalog",
    "",
...
| C | over31-plugin | z/w | 2 | v | ${isoDaysAgo(31, now)} | c |`,
    ...
    void cards;
    assert.deepEqual(
      collected.staleCards.map((entry: { plugin: string }) => entry.plugin),
      ["over31-plugin"],
    );
  });
...
describe("collectStatus rows", () => {
  ...
  it("degrades every optional row to unavailable with a producing command named", () => {
    ...
    assert.deepEqual(unavailable.map((row: { id: string }) => row.id).sort(), ["features", "plugins", "route", "smoke", "tokens"]);
...
describe("status command runner", () => {
  it("runs end to end over real files in a scratch DSH_HOME", async () => {
    ...
    const data = result.data as { rows: { id: string }[]; staleCards: unknown[] };
