...
export const credentialAccessRule = {
    id: "credential-access",
    family: "CRED",
    severity: "high",
    version: "2026.08.2",
    description: "Detects access to credential stores: ~/.claude, ~/.codex, opencode auth.json, ~/.ssh, ~/.aws, .env files, OS keychains, computed process env access via string keys, and bulk process.env enumeration.",
        ...
        return runDetectors({
            ...
            detectors: [
                ...
                },
                {
                    ...
                    pattern: /Object\s*\.\s*(?:keys|entries|values|assign)\s*\(\s*process\s*(?:\.\s*env|\[\s*['"`]env['"`]\s*\])\s*\)|\{\s*\.\.\.\s*process\s*(?:\.\s*env|\[\s*['"`]env['"`]\s*\])\s*\}|JSON\s*\.\s*stringify\s*\(\s*process\s*(?:\.\s*env|\[\s*['"`]env['"`]\s*\])/,
                ...
                {
                    code: "007",
                    pattern: /\bprocess\s*(?:\.\s*env\s*(?:\.\s*|\[\s*['"`])|\[\s*['"`]env['"`]\s*\]\s*(?:\.\s*|\[\s*['"`]))\w*(?:TOKEN|SECRET|KEY|PASSWORD|CREDENTIAL|APIKEY|API_KEY)\w*/i,
...
//# sourceMappingURL=credential-access.js.map