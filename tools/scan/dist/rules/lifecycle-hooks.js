 ...
 */
import { runDetectors, sortFindings } from "./types.js";
 ...
 */
function isBuildTimeFile(filePath) {
 ...
 */
function matchPackageJson(content, filePath, rule) {
    let parsed;
    try {
    ...
    const scripts = parsed && typeof parsed === "object" && "scripts" in parsed
        ? parsed.scripts
        : undefined;
    ...
    const present = Object.keys(scripts)
...
export const lifecycleHooksRule = {
    id: "lifecycle-hooks",
    family: "HOOK",
    severity: "medium",
    version: "2026.08.2",
    description: "Detects install-time npm lifecycle scripts and pre-consent load-time side effects (Cordis registrations, top-level timers, self-invoking network calls).",
