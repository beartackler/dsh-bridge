 ...
 */
import { lineIndexOf, makeExcerpt, maskCommentsCached, runDetectors, sha256, sortFindings, } from "./types.js";
...
const DECODE_CALL = /\b(?:atob|unescape|decodeURIComponent)\s*\(|Buffer\s*\.\s*from\s*\([^)]{0,120}?['"`](?:base64|hex)['"`]\s*\)|String\s*\.\s*fromCharCode\s*\(/;
/** Any signal that this module can execute code or reach the network. */
const EXEC_OR_NET_CAPABLE = /(?<![.\w$])eval\s*\(|\bnew\s+Function\s*\(|\(\s*0\s*,\s*eval\s*\)|\bvm\s*\.\s*run|\bchild_process\b|(?<![.\w$])fetch\s*\(|\bXMLHttpRequest\b|\bWebSocket\b|\bhttps?\s*\.\s*request\s*\(|https?:\/\/|\bset(?:Timeout|Interval)\s*\(|\baxios\b|\bnode-fetch\b|\bundici\b/;
/** High-entropy literal scan: quoted runs of base64/hex-shaped characters. */
const BLOB_LITERAL = new RegExp(`(['"\`])((?:[A-Za-z0-9+/=_-]|\\\\.){${MIN_SHORT_BLOB_LENGTH},}?)\\1`, "g");
 ...
 */
function detectHighEntropyBlobs(content, filePath, rule, haystack) {
 ...
 */
function detectStagedDecode(content, filePath, rule) {
    if (!EXEC_OR_NET_CAPABLE.test(maskCommentsCached(content)))
        return [];
    ...
    return [
        {
            ...
            family: rule.family,
            severity: "medium",
            message: "Runtime decode call inside a module that also executes code or performs network I/O; the decoded value may never appear literally in the source.",
            path: filePath,
            line,
            col,
...
export const obfuscationRule = {
    ...
    severity: "medium",
    version: "2026.08.2",
    description: "Detects concealment signals: high-entropy encoded blobs, base64/hex decode-then-execute chains (adjacent or staged through variables), obfuscator.io string-array rotation, hex identifiers, zero-width characters, homoglyphs, and minified output with no sourcemap.",
