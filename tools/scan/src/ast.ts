...
export function isAstAnalyzable(filePath: string): boolean {
 ...
 */
export function parseSourceFile(content: string, filePath: string): tsNs.SourceFile | null {
  if (!isAstAnalyzable(filePath)) return null;
  ...
  let sourceFile: tsNs.SourceFile;
  try {
...
const CREDENTIAL_PATH =
  /(?:^|[\/\\"'`.])(?:\.ssh|\.aws|\.claude|\.codex|\.netrc|\.npmrc|\.pypirc|\.git-credentials|\.gitconfig)(?:[\/\\"'`]|$)|id_(?:rsa|ed25519|ecdsa)|auth\.json|credentials\.json|\.env(?:\.[A-Za-z0-9_-]+)?["'`]?$|(?:^|\/)\.env\b/;
...
/* ------------------------------------------------------------------------- */
/* Callee / literal resolution                                                */
/* ------------------------------------------------------------------------- */

interface ResolvedName {
  /** Dotted name with computed segments resolved by constant folding, e.g. `obj.fetch`. */
  readonly dotted: string;
  /** Last segment, e.g. `fetch`. */
  readonly property: string;
  /** Leftmost identifier, e.g. `obj`. Empty when the base is not an identifier. */
  readonly root: string;
  /** True when at least one segment was written as a computed `[...]` access. */
  readonly computed: boolean;
  /** True when a computed segment needed constant folding to be readable. */
 ...
 */
function foldString(ts: TS, node: tsNs.Expression | undefined): string | undefined {
  if (!node) return undefined;
  if (ts.isStringLiteral(node) || ts.isNoSubstitutionTemplateLiteral(node)) return node.text;
  if (ts.isNumericLiteral(node)) return node.text;
  if (ts.isParenthesizedExpression(node)) return foldString(ts, node.expression);
...
function resolveName(ts: TS, node: tsNs.Expression): ResolvedName | undefined {
  const segments: string[] = [];
  let computed = false;
  let folded = false;
  let current: tsNs.Expression = node;
  ...
  for (;;) {
    ...
    if (ts.isElementAccessExpression(current)) {
      ...
      computed = true;
      if (!current.argumentExpression || !ts.isStringLiteral(current.argumentExpression)) folded = true;
...
/**
 * What a name is known to hold.
 *  - `eval` / `funcCtor`: a dynamic-execution capability.
 *  - `net`: a callable that performs egress.
 *  - `cred`: a value derived from credential material (the taint).
 *  - `decoded`: a value produced by a decode call (concealment evidence).
 *  - `fn`: a value known to be a function, used to *suppress* the timer-string finding.
 */
type Kind = "eval" | "funcCtor" | "net" | "cred" | "decoded" | "fn";

type KindMap = Map<string, Set<Kind>>;

function addKind(map: KindMap, name: string, kind: Kind): boolean {
...
const analysisCache = new WeakMap<tsNs.SourceFile, AstAnalysis>();
 ...
 */
export function analyzeSourceFile(sourceFile: tsNs.SourceFile, filePath: string): AstAnalysis {
...
function computeAnalysis(sourceFile: tsNs.SourceFile, filePath: string): AstAnalysis {
  const ts = loadTypeScript();
  if (!ts) return { findings: [] };

  const content = sourceFile.text;
  const escalate: Severity = isShippedArtifact(filePath) ? "critical" : "high";
  const kinds: KindMap = new Map();
  const emits: Emit[] = [];

  /* --- collect assignments in source order, then iterate to a fixpoint ---------- */

  interface Assignment {
    readonly name: string;
    readonly value: tsNs.Expression;
  }
  const assignments: Assignment[] = [];
  /** Names bound to a function (declaration, expression, or arrow). */
  const functionNames = new Set<string>();

  const collect = (node: tsNs.Node): void => {
    if (ts.isVariableDeclaration(node) && node.initializer && ts.isIdentifier(node.name)) {
      ...
      if (netPackage && clause) {
        if (clause.name) addKind(kinds, clause.name.text, "net");
        const bindings = clause.namedBindings;
        if (bindings && ts.isNamespaceImport(bindings)) addKind(kinds, bindings.name.text, "net");
        if (bindings && ts.isNamedImports(bindings)) {
          for (const element of bindings.elements) addKind(kinds, element.name.text, "net");
        }
      }
    }
  ...
  const classify = (node: tsNs.Expression, depth = 0): Set<Kind> => {
    const out = new Set<Kind>();
    if (depth > 12) return out;
    const merge = (other: Set<Kind>): void => {
      for (const kind of other) out.add(kind);
    };

    if (ts.isParenthesizedExpression(node)) return classify(node.expression, depth + 1);
    if (ts.isAwaitExpression(node)) return classify(node.expression, depth + 1);
    if (ts.isNonNullExpression(node) || ts.isAsExpression(node)) return classify(node.expression, depth + 1);

    if (ts.isIdentifier(node)) {
      if (node.text === "eval") out.add("eval");
      if (node.text === "Function") out.add("funcCtor");
      if (node.text === "fetch") out.add("net");
      const known = kinds.get(node.text);
      if (known) merge(known);
      if (functionNames.has(node.text)) out.add("fn");
      return out;
    }
    ...
    if (ts.isPropertyAccessExpression(node) || ts.isElementAccessExpression(node)) {
      ...
      if (resolved) {
        if (GLOBAL_ROOTS.has(resolved.root) && resolved.property === "eval") out.add("eval");
        if (GLOBAL_ROOTS.has(resolved.root) && resolved.property === "Function") out.add("funcCtor");
        if (GLOBAL_ROOTS.has(resolved.root) && resolved.property === "fetch") out.add("net");
        if (/^process(?:\.env)?$/.test(resolved.dotted.replace(/\.env\..*$/, ".env"))) {
          // handled below via base classification
        }
        if (resolved.dotted === "process.env") out.add("cred");
        if (NET_CALLEE_NAMES.has(resolved.dotted)) out.add("net");
      ...
      if (baseKinds.has("net")) out.add("net");
      if (baseKinds.has("cred")) {
        ...
        if (key !== undefined && SECRET_ENV_NAME.test(key)) out.add("cred");
        else if (key === undefined) out.add("cred"); // fully dynamic key: could be any secret
      }
      return out;
    }
    ...
    if (ts.isCallExpression(node)) {
      ...
      if (DECODE_NAMES.has(dotted) || DECODE_NAMES.has(property)) out.add("decoded");
      // `Buffer.from(x, "base64").toString()` keeps whatever x carried.
      for (const arg of node.arguments) {
        const argKinds = classify(arg, depth + 1);
        if (
          ENUMERATION_NAMES.has(dotted) ||
          ...
          if (argKinds.has("cred")) out.add("cred");
          if (argKinds.has("decoded")) out.add("decoded");
        }
      }
      if (FS_READ_NAMES.has(dotted) || FS_READ_NAMES.has(property)) {
        const first = node.arguments[0];
        if (first && CREDENTIAL_PATH.test(first.getText(sourceFile))) out.add("cred");
      }
      ...
      if (ts.isPropertyAccessExpression(node.expression) || ts.isElementAccessExpression(node.expression)) {
        ...
        if (receiver.has("cred")) out.add("cred");
        if (receiver.has("decoded")) out.add("decoded");
      }
      return out;
    }

    if (ts.isBinaryExpression(node)) {
      if (node.operatorToken.kind === ts.SyntaxKind.CommaToken) return classify(node.right, depth + 1);
    ...
    if (ts.isTemplateExpression(node)) {
      for (const span of node.templateSpans) merge(classify(span.expression, depth + 1));
      return out;
    }

    if (ts.isObjectLiteralExpression(node)) {
      for (const property of node.properties) {
        if (ts.isSpreadAssignment(property)) merge(classify(property.expression, depth + 1));
        else if (ts.isPropertyAssignment(property)) merge(classify(property.initializer, depth + 1));
        else if (ts.isShorthandPropertyAssignment(property)) merge(classify(property.name, depth + 1));
      }
      return out;
    }

    if (ts.isArrayLiteralExpression(node)) {
      for (const element of node.elements) merge(classify(element, depth + 1));
      return out;
    }

    if (ts.isNewExpression(node)) {
      const resolved = resolveName(ts, node.expression);
      if (resolved?.property === "Function") out.add("funcCtor");
      for (const arg of node.arguments ?? []) {
        if (classify(arg, depth + 1).has("cred")) out.add("cred");
      }
      return out;
    }

    if (ts.isSpreadElement(node)) return classify(node.expression, depth + 1);
  ...
  for (let round = 0; round < 8; round += 1) {
    ...
    for (const assignment of assignments) {
      for (const kind of classify(assignment.value)) {
        if (addKind(kinds, assignment.name, kind)) changed = true;
      }
    }
    if (!changed) break;
  }
  ...
  const visit = (node: tsNs.Node): void => {
    if (ts.isCallExpression(node)) {
      ...
      const dotted = resolved?.dotted ?? "";
      const property = resolved?.property ?? "";
      if (DECODE_NAMES.has(dotted) || DECODE_NAMES.has(property)) fileDecodes = true;
      ...
      if (calleeKinds.has("eval") && !isDirectEvalToken) {
        emits.push({
          ...
          severity: escalate,
          message:
            "Call resolves to eval through an alias, indirect form, or computed member; the call site does not mention eval.",
          node,
          confidence: 0.95,
      ...
      if (
        (dotted === "require" || dotted === "require.resolve") &&
      ...
      if (
        (dotted === "setTimeout" || dotted === "setInterval" || property === "setTimeout" || property === "setInterval") &&
        node.arguments.length > 0
      ) {
        const first = node.arguments[0];
        if (first !== undefined) {
          ...
          const literalFunction =
            ts.isFunctionExpression(first) || ts.isArrowFunction(first) || firstKinds.has("fn");
          if (!literalFunction) {
  ...
  const findings: Finding[] = emits.map((emit) => {
...
export function astFindingsForRule(
