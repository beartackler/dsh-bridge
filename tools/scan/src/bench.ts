...
function padTo(src: string, bytes: number): string {
  if (src.length >= bytes) return src.slice(0, bytes);
 ...
 */
function makeContent(kind: number, index: number): string {
  switch (kind % 4) {
    case 0:
      // Clean module code.
      return padTo(
    ...
    case 2:
      ...
      return padTo(
        `const w${index}={a:1,b:"ready",c:[1,2,3].map((x)=>x*2)};export default w${index};\n`,
        TARGET_BYTES,
      );
    default:
...
export function runBench(rounds: number): BenchResult {
  const root = mkdtempSync(join(tmpdir(), "dsh-scan-bench-"));
  try {
    for (let i = 0; i < FILE_COUNT; i += 1) {
      const dir = join(root, `pkg${i % 10}`);
      if (i < 10) mkdirSync(dir, { recursive: true });
      const kind = i % 4;
      const name =
        kind === 2 ? join(dir, `bundle-${i}.min.js`) : kind === 3 ? join(dir, `mod-${i}.js`) : join(dir, `src-${i}.js`);
...
function main(): void {
  ...
  process.stdout.write(
    `bench: ${r.fileCount} files x ${r.bytesPerFile} B (${(r.totalBytes / (1024 * 1024)).toFixed(1)} MB)\n` +
      `rounds: ${r.rounds}\n` +
