...
export const manifestSupplyRiskRule: Rule = {
  id: "manifest-supply-risk",
  family: "SUPPLY",
  severity: "medium",
  version: "2026.08.3",
  description:
    "Detects manifest-level supply-chain risk in package.json: dependencies pinned to git hosts or tarball URLs (mutable, unauditable sources) and install-time native-binary fetchers (prebuild-install, node-gyp, node-pre-gyp, prebuildify).",

  match(content: string, filePath: string): Finding[] {
    if (!isPackageJson(filePath)) return [];
