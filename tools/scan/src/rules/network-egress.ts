...
export const networkEgressRule: Rule = {
  ...
  version: "2026.08.2",
  description:
    "Detects outbound network capability: fetch/http/https/net/dgram/WebSocket clients, literal remote URLs, DNS lookups, third-party HTTP client imports, cloud metadata endpoints, and constructed endpoints.",
      ...
      detectors: [
        ...
        },
        {
          ...
          refine: (match) => {
            ...
            if (METADATA_HOSTS.test(host)) return false; // reported by NET-012 at critical
        ...
        },
        {
          code: "011",
          pattern: new RegExp(
            String.raw`\b(?:require\s*\(\s*|from\s+)['"](?:` + HTTP_CLIENT_PACKAGES + String.raw`)(?:/[^'"]*)?['"]`,
          ),
        ...
        },
        {
          ...
          severity: "medium",
          confidence: 0.5,
          refine: (_match, content) =>
            /\b(?:atob|unescape|decodeURIComponent)\s*\(|Buffer\s*\.\s*from\s*\([^)]*['"](?:base64|hex)['"]|String\s*\.\s*fromCharCode\s*\(/.test(
              content,
            ),
