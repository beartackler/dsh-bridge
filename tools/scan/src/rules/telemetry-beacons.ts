...
const TRACKING_SDKS =
  "posthog(?:-node)?|amplitude(?:-js)?|@sentry/(?:node|browser|electron)|mixpanel(?:-node)?|universal-analytics|segment-analytics|@segment/analytics-next|ua-parser-js";

/** Endpoints operated by known analytics collectors. */
const ANALYTICS_ENDPOINT =
  /https?:\/\/[^\s'"`\\]*(?:hm\.baidu\.com|google-analytics\.com|googletagmanager\.com|analytics\.tiktok\.com|plausible\.io|matomo|piwik|ingest(?:\.eu)?\.sentry\.io|api\.mixpanel\.com|api2\.amplitude\.com|app\.posthog\.com|cdn\.jsdelivr\.net\/.*analytics)/;

/** Generic collector paths on arbitrary hosts: the dsh-market.com heartbeat shape. */
const COLLECTOR_PATH =
  /https?:\/\/[^\s'"`\\]+\/(?:telemetry(?:\/[\w./-]+)?|collect|beacon|analytics)\b/;

function isDocFile(filePath: string): boolean {
  return /\.(?:md|markdown)$/.test(filePath);
}

export const telemetryBeaconsRule: Rule = {
  id: "telemetry-beacons",
  family: "PRIV",
  severity: "high",
  version: "2026.08.3",
  description:
    "Detects telemetry and tracking behavior: analytics SDK imports, known collector endpoints (Baidu Tongji, Google Analytics, Sentry, PostHog, Mixpanel, Amplitude), generic /telemetry /collect /beacon upload paths, and navigator.sendBeacon.",
